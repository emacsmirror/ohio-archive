...
;;  -----------
;;
;;   Conversion of blank-lines to commented lines needs work.
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defconst unibasic-rcs-version
  "@(#)$Id: unibasic.el,v 1.11 2001/01/06 01:49:55 pat Exp $"
...
a bit of room for labels. The default value is 6."
    :type 'integer
    :group 'unibasic)
  (defcustom unibasic-default-indent 3
...
It is definately preferable to have this set if possible."
    ...
    :group 'unibasic ))
 (t
  (defvar unibasic-mode-hook ()
    "*User hook for `unibasic-mode' called after the mode starts up.")
  (defvar unibasic-initial-indent 6
    "*The value for the initial indentation of unibasic statements.
 This gives a bit of room for labels. The default value is 6.")
  (defvar unibasic-default-indent 3
    "*Indentation of unibasic statements with respect to containing block.")
  (defvar unibasic-case-indent    -2
    "*Indentation offset for unibasic case statements within the case block.")
  (defvar unibasic-continuation-indent 2
    "*Indentation offset for unibasic statements continued to the next line.")
  (defvar unibasic-comment-column 40
    "*Indentation for unibasic inline comments.")
  (defvar unibasic-tab-always-indent t 
    "*Non-nil means TAB in `unibasic-mode' should always reindent the current line, regardless of where in the line the point is when a TAB is used.")
  (defvar unibasic-capitalize-keywords t    
    "*Non-nil means capitalise all keywords as they are typed.")
  (defvar unibasic-no-blank-lines t
    "*Non-nil means no blank lines are to appear in the file.
Any blank lines will be converted into a comment line by prefixing a '*' in
column 0 (as done by IPG). This doesn't work yet.")
  (defvar unibasic-use-magic-labels t
    "*Non-nil means labels after GOTO and GOSUB are made into hot-spots which
can be clicked with shift-mouse-1 taking you to the location of the label
definition.")
  (defvar unibasic-font-lock-syntactify
    (cond
     ((and (eq unibasic-emacs-type 'emacs)
           (unibasic-emacs-version-or-newer 20 0))
      t)
...
(defvar unibasic-block-start-regexp
  "\\<\\(THEN\\|ELSE\\|LOCKED\\|LOOP\\|ON ERROR\\|DO\\)[ \t]*\\($\\|;[ \t]*\\*\\)"
...
(if unibasic-mode-syntax-table
  ...
  ;; It should be something like this... but it doesn't work yet.
  ;;  (modify-syntax-entry ?\; ". 1"  unibasic-mode-syntax-table)
  ;;  (modify-syntax-entry ?\* ". 2"  unibasic-mode-syntax-table)
  ;;  (modify-syntax-entry ?\n "\> 1"   unibasic-mode-syntax-table)
...
to `unibasic-initial-indent' plus `unibasic-default-indent' as needed."
  ...
  (let* ((line (unibasic-syntactic-line 0)) ; find the last syntactic line
    ...
    (if (and (unibasic-looking-at "\\(^\\|.*;\\)[ \t]*\\<FOR\\>" line)
             (not (unibasic-looking-at 
                   "\\(^\\|.*;\\)[ \t]*\\<FOR\\>.*;[ \t]*NEXT" line)))
        (setq thiscol (+ thiscol unibasic-default-indent)))
    (if (unibasic-looking-at "\\(^\\|.*;\\)[ \t]*\\<BEGIN CASE\\>" line)
        (setq thiscol (+ thiscol unibasic-default-indent)))
    (if (unibasic-looking-at "[ \t]*CASE" line)
        (setq thiscol (- prevcol unibasic-case-indent)))
    (if (unibasic-looking-at unibasic-block-end-regexp)
        (setq thiscol (- thiscol unibasic-default-indent)))
    (cond
  ...
  (let ((col (unibasic-calculate-indent)))
    ...
    (cond
     ...
     ((looking-at unibasic-label-regexp);; labels to column 0
      ;; this might be better done by setting a flag and using a timeout
      ;; to do this. (ie: for indent-region). It's pretty quick though.
    ...
    (save-restriction
      (let ((iro inhibit-read-only)     ;; Work with read-only files
        ...
        (while (re-search-forward 
                ...
                (point-max) t)
          ;; (save-excursion
          ;;  (goto-char (point-max)) (insert (message "%s" (match-data))))
          (let ((m (match-data))
                (s 4))
            (while (elt m s)
	...
	nil
	  ...
	  (let* ((pre (previous-single-property-change (point) 'mouse-face))
		 ...
		 (txt (buffer-substring-no-properties pre pos))
            ...
            ;;(message "Matched `%s' from %d to %d" txt pre pos)
	    (if mark 
                (goto-char mark)
...
(defun unibasic-make-regexp (words)
  "Use make-regexp to turn a list of words into a usable regular expression.
 This will need some delimiters probably."
  ;; this code needs make-regexp.el to be loaded
  (if (not (fboundp 'make-regexp))
      (load-library "make-regexp"))
  (cond
   ((not (fboundp 'make-regexp))
    (message "%s" "We require make-regexp!!"))
   ((not (sequencep words))
    ())
   (t
    (let* ((regexp (make-regexp words))
           (new (make-string 0 ?x))
           (len (length regexp))
           (cn 0))
      (while (< cn len)
        (let ((c (aref regexp cn)))
          (if (char-equal c ?\\)
              (setq new (concat new "\\\\"))
            (setq new (concat new (char-to-string (aref regexp cn))))))
        (setq cn (1+ cn)))
      (concat "\\\\(" new "\\\\)")))))
...
(eval-and-compile
  (defvar unibasic-command-words
    '("$BASICTYPE" "$DEFINE" "$UNDEFINE" "$IFDEF" "$ELSE" "$ENDIF" "$IFNDEF"
      "$INCLUDE" "$INSERT"
      "ABORT" "ASSIGN" "BEGIN" "BPIOCP" "BREAK" "CALL" "CALLC" "CASE" "CHAIN"
      "CLEAR" "CLEARCOM" "CLEARCOMMON" "CLEARDATA" "CLEARFILE" "CLEARINPUT"
      "CLEARSELECT" "CLEARSQL" "CLOSE" "CLOSESEQ" "COM" "COMMENT" "COMMIT"
      "COMMON" "CONNECT" "CONTINUE" "CONVERT" "CRT" "DATA" "DEBUG" "DEFFUN"
      ...
      "FOR" "FORMLIST" "GARBAGECOLLECT" "GET" "GETCOLUMNDATA" "GETCOLUMNNAME"
...
(eval-and-compile
  (defvar unibasic-function-words
    '( "ABS" "ACOS" "ALPHA" "ASCII" "ASIN" "ATAN" "BITAND" "BITOR" "BITXOR"
       "BITNOT" "CALCULATE" "CATS" "CHANGE" "CHAR" "CHECKSUM" "COL1" "COL2"
       "CONVERT" "COS" "COUNT" "COUNTS" "DATE" "DCOUNT" "DELETE" "DIR"
       ...
       "LEN" "LENS" "LES" "LN" "LOWER" "LTS" "MATCHFIELD" "MAXIMUM" "MINIMUM"
       "MOD" "NEG" "NES" "NOT" "NOTS" "NUM" "NUMS" "OCONV" "OCONVS" "PWR"
       "QUOTE" "RAISE" "REM" "REMOVE" "REPLACE" "REUSE" "RND" "SADD" "SCMP"
       "SDIV" "SELECTINFO" "SEQ" "SEQS" "SIN" "SMUL" "SPACE" "SPACES" "SPLICE"
       "SPLICES" "SQRT" "SQUOTE" "SSUB" "STATUS" "STR" "STRS" "SUBSTRINGS"
       "SUM" "SYSTEM" "TAN" "TIME" "TIMEDATE" "TRIM" "TRIMB" "TRIMF" "TRIMS"
       "UNASSIGNED" "UPCASE" "XLATE" "XTD")))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Some debugging and testing calls. :o)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(cond
 (nil
  (define-key global-map "\C-cu" 'unibasic-mode)
  (define-key global-map "\C-cU" 'unibasic-insert-emacs-tag)
  (define-key unibasic-mode-map "\C-ci"
    (lambda () (interactive) (message "%d" (unibasic-calculate-indent))))
  (define-key unibasic-mode-map "\C-cp"
    (lambda () (interactive) (message "%d" (unibasic-syntactic-line 0))))))
