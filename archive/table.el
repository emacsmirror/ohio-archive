;;; table.el --- create and WYSIWYG edit text based embedded tables
...
;; Revised:       Tue Jan 30 2001 16:23:17 (PST)
;; Description:   create and WYSIWYG edit text based embedded tables
;; Compatibility: Emacs20.7, Emacs21.0.95, XEmacs21.1.9(some known issues)
;; URL:           http://table.sourceforge.net/

(defconst table-version "1.5.19"
...
;; missing such as coping a row/column and yanking it.  Those
...
;; -------------------
;;
;; As a default the cell contents are filled with left justification
;; and no vertical justification.  A paragraph can be justified
...
(defcustom table-tei-label-rows 1
  "*Number of top rows to play label role in TEI table."
  :tag "TEI Label Row"
  :type 'integer
  :group 'table)

(defcustom table-tei-extended nil
  "*Use extended features in TEI table."
  :tag "TEI Extended"
  :type 'boolean
  :group 'table)
...
`table-inhibit-advice' is dynamically turned on and off in the course
of table command execution.")
(defvar table-mode-indicator nil
  "For mode line indicator")
(defvar table-fixed-mode-indicator nil
  "For mode line indicator")
(defconst table-source-languages '(html latex tei cals)
...
(defvar table-advised-function-list nil
  "A list of advised functions in this package.")

(eval-and-compile
  (defconst table-advice-prefix "table-advice-"
    "Advice name prefix in this package."))

(defconst table-advice-class-property 'table-advice-class
  "A property of a function to record advice class.")
...
(defconst table-global-menu
  '("Table"
    ...
    "----"
    ["Show Version" table-version
     :active t
     :help "Show the version of the current table package"]))
...
(defmacro table-advice-simply-do-at-point-in-cache (func)
  ...
  (`
   (unless (memq '(, func) table-advised-function-list)
     (setq table-advised-function-list (cons '(, func) table-advised-function-list))
     (put '(, func) table-advice-class-property 'around)
     (defadvice (, func) (around (, (intern (concat table-advice-prefix (symbol-name func))))
...
(defun table-disable-advice ()
  "Disable all table advice by removing them from the functions."
  (interactive)
  (while table-advised-function-list
    (let ((func (car table-advised-function-list)))
      (ad-remove-advice
       func
       (get func table-advice-class-property)
       (intern (concat table-advice-prefix (symbol-name func))))
      (ad-update func)
      (setq table-advised-function-list (cdr table-advised-function-list)))))

(defun table-enable-advice ()
  "Advise functions and add them table specific behaviors.
This feature is disabled when the variable `table-disable-advising' is
non-nil."
  (interactive)
...
TAB moves the point forward by a cell. The result will look like this:
...
Type \\[table-insert-row-column] again and insert a row.  Insertion at the outside of the
table effectively appends a row at the end.
	      ...
	      (let* ((completion-ignore-case t)
			...
			(downcase (completing-read
				   (format "Existing cell contents to: (default %s) "
					   (car table-cell-split-contents-to-history))
				   '(("split") ("left") ("right"))
				   nil t nil 'table-cell-split-contents-to-history)))))
		(table--cleanup-xemacs-history 'table-cell-split-contents-to-history)
		(intern
		 (if (string= dir "")
		     (car table-cell-split-contents-to-history)
		   dir)))))
...
'center, 'right, 'top, 'middle, 'bottom or 'none."
  (interactive
   (list (intern (let* ((dummy (barf-if-buffer-read-only))
			(completion-ignore-case t)
			(what (downcase (completing-read
					 (format "Justify what: (default %s) "
						 (car table-target-history))
					 '(("cell") ("row") ("column"))
					 nil t nil 'table-target-history))))
		   (table--cleanup-xemacs-history 'table-target-history)
		   (if (string= what "")
		       (car table-target-history)
		     what)))
...
'middle, 'bottom or 'none for vertical."
  (interactive
   (list (table--query-justification)))
  (let((table-inhibit-advice t)
       (cell-list (table--horizontal-cell-list nil nil 'top)))
    (table--finish-delayed-tasks)
    (save-excursion
      (while cell-list
...
'middle, 'bottom or 'none for vertical."
  (interactive
   (list (table--query-justification)))
  (let((table-inhibit-advice t)
       (cell-list (table--vertical-cell-list nil nil 'left)))
    (table--finish-delayed-tasks)
    (save-excursion
      (while cell-list
	(let ((cell (car cell-list)))
  ...
  (interactive
   (let ((language))
     ...
     (list
      (intern
       (let ((completion-ignore-case t))
	 (setq language (downcase (completing-read
				   (format "Language: (default %s) "
					   (car table-source-language-history))
				   (mapcar (lambda (s) (list (symbol-name s)))
					   table-source-languages)
				   nil t nil 'table-source-language-history)))
	 (table--cleanup-xemacs-history 'table-source-language-history)
	 (if (string= language "")
	     (setq language (car table-source-language-history))
	   language)))
      (read-buffer "Destination buffer: " (concat table-dest-buffer-name "." language))
      (read-from-minibuffer "Table Caption: "
			    (car table-source-caption-history) nil nil 'table-source-caption-history))))
...
(defun table--generate-source-prologue (dest-buffer language caption col-list row-list)
  ...
  (with-current-buffer dest-buffer
    (cond
     ((eq language 'html)
      (insert (format "<!-- This HTML table template is generated by `table.el' version %s -->\n"
		      table-version)
		...
		"")))
     ((eq language 'latex)
      (insert (format "%% This LaTeX table template is generated by `table.el' version %s\n"
		      table-version)
	      "\\begin{tabular}{|" (apply 'concat (make-list (length col-list) "l|")) "}\n"
	      "\\hline\n"))
     ((eq language 'tei)
      (insert (format "<!-- This TEI table template is generated by `table.el' version %s -->\n" table-version)
	      "<table rend=\"frame\">\n")
      (if (and (stringp caption)
	       (not (string= caption "")))
	  (insert "  <head>" caption "</head>\n")))
     ((eq language 'cals)
      (insert (format "<!-- This CALS table template is generated by `table.el' version %s -->\n" table-version)
	      "<table frame=\"all\">\n")
      (if (and (stringp caption)
	       (not (string= caption "")))
...
(defun table--generate-source-scan-rows (dest-buffer language origin-cell col-list row-list)
  ...
  (while row-list
	...
	(insert "  <TR>\n"))
       ((eq language 'tei)
	(insert "  <row")
        (if (<= (table-get-source-info 'current-row) table-tei-label-rows)
	    (insert " role=\"label\""))
	(insert ">\n"))
       ((eq language 'cals)
	(insert "      <row>\n"))
	...
	(if (< (car lu) (car col-list))
	  ...
	  (with-current-buffer dest-buffer
	    (cond
	     ...
	     ((eq language 'tei)
	      (insert "    <cell")
	      (if (> colspan 1) (insert (format " cols=\"%d\"" colspan)))
	      (if (> rowspan 1) (insert (format " rows=\"%d\"" rowspan)))
	      (if table-tei-extended
		  (progn
		    (if (and valign
			     (not (memq valign '(top none))))
			(insert (format " valign=\"%s\"" (symbol-name valign))))
		    (if (and alignment
			     (not (memq alignment '(left))))
			(insert (format " halign=\"%s\"" (symbol-name alignment))))))
	      (insert ">\n"))
	     ((eq language 'cals)
	      (insert "        <entry")
	      (if (> colspan 1)
	  ...
	  (let ((span 1)
	    ...
	    (while (setq c (nth i border-char-list))
		  ...
		  (let ((line (table--buffer-substring-and-trim
			       (table--goto-coordinate (cons start y))
			       (table--goto-coordinate (cons (1- (nth i col-list)) y)))))
			...
			(insert "&"))
		      (if (> span 1)
			  (insert (format "\\multicolumn{%d}{l|}{%s}" span line))
			(insert line)))
		    (setq span 1))
		(setq span (1+ span)))
	      (setq i (1+ i)))
	    (let ((line (table--buffer-substring-and-trim
			 (table--goto-coordinate (cons start y))
			 (table--goto-coordinate (cons x1 y)))))
	      (with-current-buffer dest-buffer
		(if first-p
		    (setq first-p nil)
		  (insert "&"))
...
"
     ...
     (list (read-from-minibuffer
	   ...
	   (string-to-number
	    (read-from-minibuffer
	     "How many elements: (0: maximum, negative: backward traveling) "
	     (car table-sequence-count-history) nil nil 'table-sequence-count-history))
	   (string-to-number
	    (read-from-minibuffer
	     "Increment element by: "
	     (car table-sequence-increment-history) nil nil 'table-sequence-increment-history))
	   (string-to-number
	    (let ((str
		   (read-from-minibuffer
		    "Cell interval: (0: vertical, 1:horizontal) "
		    (car table-sequence-interval-history) nil nil 'table-sequence-interval-history)))
	      (if (string= str "") "1" str)))
	   (let* ((completion-ignore-case t)
		  (justify (downcase (completing-read
				      (format "Justify: (default %s) "
					      (car table-sequence-justify-history))
				      '(("left") ("center") ("right"))
				      nil t nil 'table-sequence-justify-history))))
	     (table--cleanup-xemacs-history 'table-sequence-justify-history)
	     (intern
	      (if (string= justify "")
		  (car table-sequence-justify-history)
		justify))))))
  ...
  (if (interactive-p)
      (message "Sequencing..."))
  (let* ((table-inhibit-advice t)
	 (prefix (substring str 0 (match-beginning 1)))
  ...
  (interactive
   (let ((col-delim-regexp)
     ...
     (list
      ...
      (let* ((completion-ignore-case t)
	     (justify
	      (if (and (string= col-delim-regexp "") (string= row-delim-regexp "")) nil
		(downcase (completing-read
			   (format "Justify: (default %s) "
				   (car table-capture-justify-history))
			   '(("left") ("center") ("right"))
			   nil t nil 'table-capture-justify-history)))))
	(if (null justify) 'left
	  (table--cleanup-xemacs-history 'table-capture-justify-history)
	  (intern
	   (if (string= justify "")
	       (car table-capture-justify-history)
	     justify))))
      (if (and (string= col-delim-regexp "") (string= row-delim-regexp "")) 1
	(read-from-minibuffer "Minimum cell width: "
			      (car table-capture-min-cell-width-history) nil nil 'table-capture-min-cell-width-history))
      (if (and (not (string= col-delim-regexp "")) (string= row-delim-regexp ""))
	  (string-to-number
	   (read-from-minibuffer "Number of columns: "
				 (car table-capture-columns-history) nil nil 'table-capture-columns-history))
	nil)
      )))
...
(defun *table--cell-dabbrev-expand (arg)
  ...
  (interactive "*P")
  (let ((table-inhibit-advice t)
	(dabbrev-abbrev-char-regexp (concat "[^"
...
(defun table--buffer-substring-and-trim (beg end)
  ...
  (save-excursion
    (goto-char beg)
    (if (looking-at "\\s +") (setq beg (match-end 0)))
    (goto-char beg)
    (let ((bound end))
...
looking at the appearance of the CELL contents."
  (let ((cell-contents (extract-rectangle (car cell) (cdr cell)))
	...
	(let* ((lm (- (match-end 1) (match-beginning 1)))
      ...
      (goto-char (point-min))
      (if (looking-at "\\s *\n")
	  (setq top-margin (count-lines (match-beginning 0) (match-end 0))))
	 ...
	 ((vectorp elt)
	  (let ((i 0)
	    ...
	    (while (< i len)
	      ...
	      (setq i (1+ i)))))
	 ((consp elt)
	  ;; strip the menu bar and tool bar items
	  (if (or (eq (car elt) 'menu-bar);; new format menu item
		  (stringp (car elt));; old format menu item
		  (eq (car elt) 'tool-bar));; tool bar menu item under Emacs 21
	      (progn
    ...
    (cond ((and (symbolp binding)
	   ...
	   (or (cdr (assoc binding table-cell-global-map-alist))
	       (let ((symbol (intern (concat "table-cell-map-" (symbol-name binding))))
		     (func (symbol-function binding)))
		 (setq table-cell-global-map-alist
		       (cons (cons binding symbol) table-cell-global-map-alist))
		 (if (eq (car func) 'autoload) ;; if it is autoload type load it
		     (load-library (cadr func)))
...
Refresh the menu bar."
  (if table-cell-entered-state nil
