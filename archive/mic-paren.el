...
;; parenthesises) is highlighted until the cursor is moved away from the
...
;;   (vertical and/or horizontal). Message contains the either the linenumber
;;   or the number of lines between the two matching parens. Option to select
;;   in which cases this message should be displayed.
...
;;
;; This file can be obtained from "The Ohio Emacs Lisp State Archive"
...
;;           has no preceeding text in the same line.
...
;; TODO:
;;
;; + mic-paren-horizontal-pos-visible-p seems to have a very little bug
;;   (in 99.99% it should work, but maybe the limit-values in the
;;   comparisons differ from the correct value by 1 or 2; normally this
;;   doesn´t matter!)

;;; Code

(defvar mic-paren-version "3.3"
...
Normaly you don´t want to change the default-value!"
 :set (function
       (lambda (symbol value)
...
The following options are available via the customize-feature:
  ...
  `paren-no-match-face'
  `paren-bind-modified-sexp-functions'
The following options are setable via toggling functions \(look at the
