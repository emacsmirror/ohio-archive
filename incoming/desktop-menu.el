...
;; Copyright (C) 1999 Olaf Sylvester
...
;; This packages provides a Desktop Menu for managing differnt Emacs desktops
;; in a directory.
;; 
;; Start menu with
;; M-x desktop-menu
...
(defun desktop-menu-initialise ()
  "Create default directory for differnet desktop files."
...
`desktop-menu-list-file' for desktop list."
  ...
  (if read-main-p
      (setq desktop-menu-desktops (desktop-menu-read-main-list directory)))
  (desktop-menu-mode)
  (let ((inhibit-read-only t)
	...
	(insert (if (string= (car desktop) desktop-menu-current-desktop-name)
		    "." " ")
		(format "  %-30s %s" (car desktop) (car (cdr desktop))))
	(newline))
      (setq desktops (cdr desktops)))
...
(defun desktop-menu-change-directory (directory)
  ...
  (interactive "DChnage to directory: ")
...
Optional argument CLEAR-P `ask', `yes' or `no'.
