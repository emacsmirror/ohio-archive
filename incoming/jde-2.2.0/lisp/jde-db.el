...
;; $Revision: 1.63 $ $Date: 2000/06/12 08:37:43 $ 
...
(makunbound 'jde-db-debugger)
(defcustom jde-db-debugger (cons "Other" (cons "jdb" "Executable"))
"Specify the debugger you want to use to debug Java programs.
Select JDEbug, if you want to use the JDE's builtin debugger. Otherwise,
select Other Other may be an executable, like jdb, or a Java class. If Other 
is a class, the JDE launches a vm to run the debugger. If you choose Other,
 select the debugger's type (executable or class) and enter the path name 
of the debugger, if Other is an executable, or a  fully qualified package name,
if Other is a class."
  :group 'jde-project
  :type '(cons
...
;; package names to begin underscores.
...
(defun jde-db-contains-package-p (dir package)
   ...
   (let ((pkg-path (jde-db-pkg-to-path package)))
     (file-exists-p (expand-file-name pkg-path dir))))

; (defun jde-test (dir package)
;   (interactive
;    "sDir: \nsPackage: ")
;   (message (if (jde-db-contains-package-p dir package) "true" "false")))
...
(defun jde-db-init (app-class marker-filter find-file)
  (let ((debug-buf-name (concat "*debug" app-class "*"))
	...
	(let* ((debug-buffer (get-buffer-create debug-buf-name))
	       (program (if (string= (cdr (cdr jde-db-debugger)) "Executable")
			   (car (cdr jde-db-debugger))
			 jde-run-java-vm))
	       (prog-args (if (string= (cdr (cdr jde-db-debugger)) "Executable")
			      (append 
			       (jde-db-get-vm-args)
