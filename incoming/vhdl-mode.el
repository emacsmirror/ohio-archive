...
;;              Rodney J. Whitby <rwhitby@geocities.com>
;; Maintainer:  VHDL Mode Maintainers <vhdl-mode@emacs.org>
;; Version:     3.30
;; Keywords:    languages vhdl
;; WWW:         http://www.emacs.org/hdl/vhdl-mode.html
...
;; supported: GNU Emacs 20.X, XEmacs 20.X/21.X
...
;;   Emacs:  M-x byte-compile-file  ->  vhdl-mode.el
...
;; help function for user variables
...
;; help string for user variables
...
(defcustom vhdl-entity-file-name '(".*" . "\\&.vhd")
  (concat
   "*Specifies how the entity file name is obtained.
The entity file name can be obtained by modifying the name of the entity (e.g.
attaching or stripping off a substring).
Used for sensitivity list updating (obtaining port names)."
...
The following keywords for template generation are supported:
  ...
  <author>     : replaced by the user name and email address (`user-full-name',
...
header keywords (see variable `vhdl-file-header')."
  :type 'string
  :group 'vhdl-header)
...
Hierarchy (instances of subcomponents):
  ...
  Instances per file: maximum number of instances to scan per file (in bytes)
...
(defun vhdl-warning-when-idle (string)
  "Wait until idle, then print out wanring STRING and beep."
  (if vhdl-xemacs
      (start-itimer "vhdl-mode" 'vhdl-warning .1 nil t t string)
    (run-with-idle-timer .1 nil 'vhdl-warning string)))
...
(defun vhdl-print-warnings ()
  ...
  (when vhdl-startup-warnings (beep))
...
;; Compatibility checks and corrections
;; variable `vhdl-compiler' changed format
(when (not (stringp vhdl-compiler))
  (setq vhdl-compiler "ModelSim")
  (vhdl-add-warning "Variable `vhdl-compiler' has changed format; customize again"))
;; variable `vhdl-standard' changed format
(when (not (listp vhdl-standard))
  (setq vhdl-standard '(87 nil))
  (vhdl-add-warning "Variable `vhdl-standard' has changed format; customize again"))
;; variable `vhdl-model-alist' changed format
...
;; variable `vhdl-project-alist' changed format
...
(require 'easymenu)
(when vhdl-xemacs (require 'hideshow))	; XEmacs needs it, Emacs gives an error
(require 'hippie-exp)
...
;; `wildcard-to-regexp' is included in XEmacs 21
...
(defun vhdl-mode-map-init ()
  ...
  (define-key vhdl-mode-map "\C-c\C-a\M-a" 'vhdl-align-noindent-region)
  (define-key vhdl-mode-map "\C-c\C-a\C-i" 'vhdl-align-inline-comment-group)
  (define-key vhdl-mode-map "\C-c\C-a\M-i" 'vhdl-align-inline-comment-region)
...
(defun vhdl-create-mode-menu ()
  ...
  (list
   ...
   '("Align"
     ...
     "--"
     ["Inline Comment Group"	vhdl-align-inline-comment-group
				:keys "C-c C-a C-i"]
...
- ALIGNMENT:  The alignment functions align operators, keywords, and inline
  comments to beautify the code.  `\\[vhdl-align-group]' aligns a group of consecutive
  lines separated by blank lines.  `\\[vhdl-align-list]' aligns all lines belonging
  to a list enclosed by a pair of parentheses (e.g. port clause/map, argument
  list).  `\\[vhdl-align-declarations]' aligns all lines within the declarative part of a
  design unit.  `\\[vhdl-align-noindent-region]' aligns an entire region.  `\\[vhdl-align-inline-comment-group]' aligns
  inline comments for a group of lines, and `\\[vhdl-align-inline-comment-region]' for a region.
  If variable `vhdl-align-groups' is non-nil, groups of code lines separated
  by empty lines are aligned individually.  Some templates are automatically
...
- PORT TRANSLATION:  Generic and port clauses from entity or component
  ...
  parameters, and as a test bench (menu).  To include formals in component
  instantiations, see variable `vhdl-association-list-with-formals'.  To
  include comments in pasting, see variables `vhdl-include-...-comments'.
...
- FILE BROWSER:  The speedbar allows browsing of directories and file contents.
  ...
  variable `vhdl-speedbar-auto-open' is non-nil.
...
- HIGHLIGHTING (fontification):  Keywords and standardized types, attributes,
  ...
  For documentation and customization of the used colors see customization
  group `vhdl-highlight-faces' (`\\[customize-group]').
...
- HIDE/SHOW:  The code of blocks, processes, subprograms, component
  declarations and instantiations, generic/port clauses, and configuration
  declarations can be hidden using the `Hide/Show' menu or by pressing
  `S-mouse-2' within the code (see customization group `vhdl-menu').
  XEmacs: limited functionality due to old `hideshow.el' package.

- CODE UPDATING:
  - Sensitivity List: `\\[vhdl-update-sensitivity-list-process]' updates the sensitivity list of the
    current process, `\\[vhdl-update-sensitivity-list-buffer]' of all processes in the current buffer.
    Limitations:
      - Only declared local signals (ports, signals declared in architecture
        and blocks) are automatically inserted.
    ...
    Use variable `vhdl-entity-file-name' to specify the entity file name (used
    to obtain the port names).

- CODE FIXING:  `\\[vhdl-fix-clause]' fixes the closing parenthesis of a
  generic/port clause (e.g. if the closing parenthesis is on the wrong line
  or is missing).
...
- RELEASE NOTES:  See also the release notes (menu) for added features in new
  releases.
...
Questions and enhancement requests can be sent to <vhdl-mode@emacs.org>.
...
mailing lists by sending an email to <vhdl-mode@emacs.org>.

VHDL Mode is officially distributed at http://www.emacs.org/hdl/vhdl-mode.html
...
;  (set (make-local-variable 'lazy-lock-defer-time) 0.1)
  ...
  (message "VHDL Mode %s.  See menu for documentation and release notes."
	   vhdl-version)
  (vhdl-print-warnings)
  ...
  (vhdl-prepare-search-2
   (cond ((= (char-syntax (preceding-char)) ?w)
	 ...
	 ((and (or (eq last-command 'vhdl-electric-tab)
	  ...
	  (if (looking-at "--")
	      (if (< (current-indentation) comment-column)
		  (progn (indent-to comment-column) (vhdl-indent-line))
		(kill-line -0) (indent-to 1) (vhdl-indent-line))
...
(defun vhdl-align-list (&optional spacing)
  "Align the lines of a list surrounded a balanced group of parentheses."
  (interactive)
  (let (beg end)
    (save-excursion
	...
	(setq end (point))))
    (if beg
	(vhdl-align-noindent-region beg end spacing)
      (error "Not within a list enclosed by a pair of parentheses"))))
...
(defun vhdl-align-declarations (&optional spacing)
  ...
  (let (beg end)
    ...
    (if beg
	(vhdl-align-noindent-region beg end spacing)
      (error "Not within the declarative part of a design unit"))))
...
(defun vhdl-update-sensitivity-list ()
  "Update sensitivity list."
	...
	(error "Process with wait statement, sensitivity list not generated"))
       ;; look if sequential process (sensitivity list not updated)
;        ((progn (goto-char proc-mid)
; 	       (vhdl-re-search-forward "\\('event\\|\\<\\(falling\\|rising\\)_edge\\)\\>" proc-end t))
; 	(error "Sequential process, sensitivity list not updated"))
	...
	(let
	  ...
	  (if (not (looking-at "[ \t\n]*("))
	    ...
	    (while (and (re-search-forward "\\(\\w+\\)" sens-end t)
			(setq sens-list (cons (match-string 0) sens-list))
	  ...
	  (while scan-regions-list
		...
		(while (re-search-forward "\\([a-zA-Z]\\w*\\)" end t)
		  (setq name (match-string 1))
		  (when (and (member name signal-list)
			     (not (member name read-list)))
	  ...
	  (setq read-list (sort read-list 'string-lessp))
	  (when read-list
	...
	(while (and (progn (while (and (setq beg (re-search-backward "^\\s-*\\(\\w+\\s-*:\\s-*block\\|\\(end\\)\\s-+block\\)\\>" nil t))
	  ...
	  (while (re-search-forward "^\\s-*signal\\>" end t)
	    (while (and (not (looking-at "[ \t\n]*:"))
			(re-search-forward "[ \t\n,]+\\(\\w+\\)" end t))
	      (setq signal-list (cons (match-string 1) signal-list))))
	  (goto-char beg)))
	  ...
	  (error "No architecture declarative part found")
	;; scan for all declared signal names
	(goto-char beg)
	(while (re-search-forward "^\\s-*signal\\>" end t)
	  (while (and (not (looking-at "[ \t\n]*:"))
      ...
      (goto-char (point-min))
      (when (not (re-search-forward (concat "^entity\\s-+" entity-name "\\>") nil t))
	(setq file-name
	      (vhdl-replace-string vhdl-entity-file-name entity-name)))
      (vhdl-visit-file
       file-name
       '(progn
	  (vhdl-prepare-search-2
	   ...
	   (if (not (re-search-forward (concat "^entity\\s-+" entity-name "\\>") nil t))
	       (progn
		 (message "  --> see custom variable `vhdl-entity-file-name'")
		 (error "Entity \"%s\" not found (see *Messages* buffer)"
			entity-name))
  ...
  (save-excursion
    (vhdl-prepare-search-2
     (let ((pos (point))
	   ...
	   (error "Not within a generic/port clause")
    ...
    (when (setq arch-name
      ...
      (if (save-excursion
	    (vhdl-prepare-search-1
	     (re-search-backward "\\<entity \\(\\w+\\) is\\>" nil t)))
...
(defun vhdl-template-header ()
  ...
  (unless (equal vhdl-file-header "")
    (let ((project-name (or (nth 0 (aget vhdl-project-alist vhdl-project)) ""))
	 ...
	 (save-restriction
	   ...
	   (goto-char (point-min))
	   (while (search-forward "<copyright>" nil t)
...
(defun vhdl-template-package (&optional kind)
  ...
  (let ((margin (current-indentation))
    ...
    (when body
      ...
      (when (save-excursion
	      (vhdl-prepare-search-1
	       (re-search-backward "\\<package \\(\\w+\\) is\\>" nil t)))
...
(defun vhdl-insert-string-or-file (string)
  ...
  (unless (equal string "")
    (cond ((file-exists-p (progn (string-match "^\\([^\n]+\\)"string)
...
(defun vhdl-port-parse-group-comment ()
...
(defun vhdl-port-paste-group-comment (string indent)
  ...
  (save-excursion
	...
	(catch 'parse
	  ...
	  (when (vhdl-parse-string "generic[ \t\n]*(" t)
	    ...
	    (setq group-comment (vhdl-port-parse-group-comment))
	  ...
	  (when (vhdl-parse-string "port[ \t\n]*(" t)
		...
		(setq comment (and (vhdl-parse-string "--\\s-*\\([^\n]*\\)" t)
				   (match-string 1))))
	      ;; save everything in list
	      (setq ports (append ports (list (list names object direct
						    type comment group-comment))))
	      ;; parse group comment and spacing
	      (setq group-comment (vhdl-port-parse-group-comment))))
	  nil)))
      ;; finish parsing
...
(defun vhdl-port-paste-port ()
  ...
  (let ((margin (current-indentation))
	...
	;; paste group comment and spacing
	(when (memq vhdl-include-group-comments '(decl always))
	  (vhdl-port-paste-group-comment (nth 5 port) list-margin))
	;; paste object
  ...
  (if (not vhdl-port-list)
	...
	(when generics-list
	  ...
	  (while generics-list
	    ...
	    ;; paste group comment and spacing
	    (when (memq vhdl-include-group-comments '(decl always))
	      (vhdl-port-paste-group-comment (nth 4 generic) margin))
	    (vhdl-insert-keyword "CONSTANT ")
...
(defun vhdl-port-paste-signals (&optional initialize)
  ...
  (if (not vhdl-port-list)
	...
	(while ports-list
	  ...
	  ;; paste group comment and spacing
	  (when (memq vhdl-include-group-comments '(decl always))
	    (vhdl-port-paste-group-comment (nth 5 port) margin))
	  ;; paste object
	  (if (nth 1 port)
	      (insert (nth 1 port) " ")
	    (vhdl-insert-keyword "SIGNAL "))
...
(defun vhdl-re-search-forward (regexp &optional bound noerror count)
  ...
  (let (pos)
    (save-excursion
      (setq pos (re-search-forward regexp bound noerror count))
      (while (and (vhdl-in-literal)
		  (setq pos (re-search-forward regexp bound noerror count)))))
    (when pos (goto-char pos))
    pos))

(defun vhdl-visit-file (file-name body)
  ...
  (if (null file-name)
	...
	(unless (file-exists-p file-name)
	  (vhdl-warning (concat "File not found: \"" file-name "\"") t))
...
`vhdl-compiler'."
  ...
  (let* ((command-elem (assoc vhdl-compiler vhdl-compiler-alist))
	 ...
	 (vhdl-compile-file-name (buffer-file-name))
	 compilation-process-setup-function)
    ;; print out file name if compiler does not
    (when (and (= 0 (nth 1 (nth 5 command-elem)))
	       (= 0 (nth 1 (nth 6 command-elem))))
...
;; does not work well
;; problem: `hs-already-hidden-p' if hiding more than start regexp matches
; (defun vhdl-hs-adjust-block-beginning (pos)
;   "Adjust the block beginning for hiding."
;   (save-excursion
;     (let ((case-fold-search t))
;       (vhdl-prepare-search-2
;        (goto-char pos)
;        (backward-word 2)
;        (if (looking-at "\\(generic\\|port\\)\\s-+map\\>")
; 	   (progn (skip-chars-backward " \t\n")
; 		  (point))
; 	 pos)))))

;; add VHDL hideshow properties to `hs-special-modes-alist'
(let ((start
       (concat
	"\\(^\\)\\s-*\\("
	;; generic/port clause
	...
	;; component instantiation
	"\\(\\w\\|\\s_\\)+\\s-*:[ \t\n]*"
	"\\(\\(component\\|COMPONENT\\|configuration\\|CONFIGURATION\\|entity\\|ENTITY\\)\\s-+\\)?"
	"\\(\\w\\|\\s_\\)+\\(\\s-*(\\(\\w\\|\\s_\\)+)\\)?[ \t\n]*"
	"\\(generic\\|GENERIC\\|port\\|PORT\\)\\s-+\\(map\\|MAP\\)[ \t\n]*(\\|"
	;; subprogram
	"\\(function\\|FUNCTION\\|procedure\\|PROCEDURE\\)\\>\\|"
	;; process, block
	"\\(\\(\\w\\|\\s_\\)+\\s-*:\\s-*\\)?\\(process\\|PROCESS\\|block\\|BLOCK\\)\\>\\|"
	;; configuration declaration
	"\\(configuration\\|CONFIGURATION\\)\\>"
	"\\)"))
      (end nil)
      (comment-start "--\\( \\|$\\)")
      (forward-sexp-func 'vhdl-hs-forward-sexp-func)
      (adjust-beg-func nil))		;'vhdl-hs-adjust-block-beginning))
  (setq hs-special-modes-alist
	(cons (if (= 3 (length (car hs-special-modes-alist))) ; old package
		  (list 'vhdl-mode start end forward-sexp-func)
		(list 'vhdl-mode start end comment-start
		      forward-sexp-func adjust-beg-func))
...
(defun vhdl-hideshow-init ()
  ...
  ;; override `hs-find-block-beginning'
  (add-hook 'hs-minor-mode-hook
	    '(lambda () (defalias 'hs-find-block-beginning
			  'vhdl-hs-find-block-beginning)))
...
;; used to override `hs-find-block-beginning' in `hideshow.el':
;; - implements different search strategy:
;;   - hideshow:  search backward for block beginnings or ends; within block
;;                if beginning found
;;   - vhdl-mode: search backward for block beginning, goto block end; within
;;                block if cursor before block end
;; - allows hiding of blocks with no unique block ending (e.g. closing
;;   parenthesis of generic/port clauses and component instantiations)
(defun vhdl-hs-find-block-beginning ()
  "Reposition point at block-start. Return point, or nil if top-level.
Overrides existing `hs-find-block-beginning' function."
  (let (done
	(here (point)))
      (beginning-of-line)
	...
	(while (and (re-search-backward hs-block-start-regexp nil t)
		    (not (setq done (<= here (save-excursion
  ...
  (list
   ...
   (list
    (concat
     ...
     "\\(\\w+\\)\\.\\(\\w+\\)\\((\\(\\w+\\))\\)?")
    '(3 font-lock-function-name-face) '(4 font-lock-function-name-face)
    '(6 font-lock-function-name-face nil t))
...
;     (message "No such directory: \"%s\"" (match-string 1 name)))
  (let* ((is-directory (= (match-beginning 2) (match-end 2)))
	 ...
	 (source-buffer (current-buffer))
	 ent-alist pack-alist ent-inst-list no-files big-files)
	...
	(while file-list
	  ...
	  (let ((file-name (abbreviate-file-name (car file-list)))
		...
		ent-list arch-list conf-list pack-list inst-list inst-no
		...
		(while (re-search-forward
		  ...
		  (let* ((ent-entry (aget ent-alist (match-string 2)))
			 (arch-alist (nth 2 ent-entry))
			 (conf-alist (nth 3 ent-entry)))
		    (setq arch-name (match-string 1))
		    (setq ent-name (match-string 2))
		...
		(while (re-search-forward
		  ...
		  (progn (message "WARNING:  Scan limit (hierarchy: file size) reached in file:\n  \"%s\"" file-name)
			 (setq big-files t))
		;; scan for instantiations and the enclosing architecture
		(goto-char (point-min))
		(setq inst-no 0)
		(while (and (or (null limit-hier-inst-no)
			    ...
			    (re-search-forward
			     (concat
			      ...
			      "\\(\\w+\\)\\s-*:\\(\\s-\\|\n\\)*\\(entity\\s-+\\w+\\.\\)?"
			      "\\(\\w+\\)\\(\\s-*(\\(\\w+\\))\\)?\\(\\s-\\|\n\\|--.*\n\\)*"
			      "\\(generic\\|port\\)\\s-+map\\>\\)") nil t))
		  (if (match-string 2)
		      ;; architecture found
		      (progn (setq arch-name (match-string 2))
		...
		(when (and limit-hier-inst-no (<= limit-hier-inst-no inst-no))
		  (message "WARNING:  Scan limit (hierarchy: instances per file) reached in file:\n  \"%s\"" file-name)
		  (setq big-files t))
	...
	(when big-files
	  (message "  --> see custom variable `vhdl-speedbar-scan-limit'")
	  (vhdl-warning-when-idle "Scanning is incomplete (see *Messages* buffer)"))
	t))))
...
(defun vhdl-speedbar-find-file (text token indent)
  ...
  (if (not (car token))
    ...
    (when vhdl-speedbar-jump-to-unit
      (goto-line (cdr token))
      (recenter))
...
(defun vhdl-speedbar-port-copy ()
  ...
  (if (re-search-forward "\\([0-9]\\)+:\\s-*\\[[-+?]\\] \\(\\(\\w\\|\\s_\\)+\\)"
	...
	(let* ((indent (string-to-number (match-string 1)))
	  ...
	  ;; visit file and scan port
	  (vhdl-visit-file file-name '(progn (goto-line (nth 1 ent-entry))
					     (end-of-line)
					     (vhdl-port-copy))))
...
(defconst vhdl-mode-help-address "vhdl-mode@emacs.org"
...
(defvar vhdl-doc-release-notes nil
  "\
Release Notes for VHDL Mode 3.30
================================

New features
------------

- Hide/Show:  Massively enhanced usage of `hideshow.el'.
    Allows hiding of blocks, processes, subprograms, component declarations
    and instantiations, generic/port clauses, and configurations declarations.
    Limited functionality available in XEmacs (old `hideshow.el' package).

- Code updating:  Automatically update the sensitivity list of processes.

- Code fixing:  Fix closing parenthesis of generic/port clauses.

- Port pasting:  Include group comments, empty lines and type (instantiations).

- Hierarchy scanning:  Size limits to prevent scanning of large netlist files.

- Alignment:  Additional alignment functions, changed key bindings.

- Moving:  Functions to move to line with same indent.

- Highlighting:  Formal parameters and names in library/use clauses added.


Custom variables
----------------

`vhdl-copyright-string' (new):
    Copyright string to be inserted in file header.
`vhdl-entity-file-name' (new):
    Name of entity file (used for sensitivity list updating).
`vhdl-include-group-comments' (new):
    Include group comments in port pasting.
`vhdl-include-type-comments' (new):
    Include type in comments of instance pastings.
`vhdl-speedbar-auto-open' (changed from `vhdl-speedbar'):
    Automatically open speedbar at startup.
`vhdl-speedbar-jump-to-unit' (new):
    Disable jumping to design unit when opening buffer from speedbar.
`vhdl-speedbar-scan-limit' (new):
    Limit file size and number of instances for hierarchy scanning.")
