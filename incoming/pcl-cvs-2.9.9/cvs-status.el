...
;; Version: v2_9_9
;; Revision: cvs-status.el,v 1.14 2000/03/05 21:32:21 monnier Exp
...
(cvs-defmap cvs-status-mode-map
  '(("n"	. next-line)
    ("p"	. previous-line)
    ("\M-n"	. cvs-status-next-entry)
    ("\M-p"	. cvs-status-prev-entry)
    ;;("g"	. cvs-status-cmd-force-update)
    ;;("."	. cvs-status-cmd-show-tag-name)
    ;;(">"	. cvs-status-cmd-checkout)
    ;;("?"	. cvs-status-cmd-log)
...
(cvs-define-major-mode "CVS-Status"
...
(eval-when-compile (autoload 'font-lock-fontify-region "font-lock"))
