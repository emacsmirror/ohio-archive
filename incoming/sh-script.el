...
;; Modifications: Reto Zimmermann <reto@gnu.org>, indentation added 99/12/09
;; Version: 2.1a
;; Maintainer: FSF
...
(defvar sh-mode-menu-list
  '("Shell"
    ...
    "--"
    ["(Un)Comment Out Region"	sh-comment-uncomment-region (mark)]
    "--"
    ["Indent Line"		sh-indent-line t]
    ["Indent Region"		sh-indent-region (mark)]
    ["Indent Buffer"		sh-indent-buffer t]
    )
  "Sh Mode menu.")
...
(defvar sh-mode-map
  (let ((map (make-sparse-keymap))
    ...
    (define-key map "\M-\C-\\" 'sh-indent-region)
    (define-key map "\C-cc" 'sh-comment-uncomment-region)
    (define-key map "\t" 'sh-electric-tab)
    (define-key map "}" 'sh-electric-closing-brace)
    (define-key map "\r" 'sh-electric-return)
    (define-key map " " 'sh-electric-space)
...
with your script for an edit-interpret-debug cycle."
  ...
  (sh-init-indent)
  (turn-on-font-lock)
  (easy-menu-add sh-mode-menu-list) ; for XEmacs
  (easy-menu-define sh-mode-menu sh-mode-map
		    "Menu keymap for Sh Mode." sh-mode-menu-list)
...
;; indentation
(defconst sh-beg-block-re-alist
  '((csh "{\\|\\<\\(foreach\\|if\\>.*\\<then\\|switch\\|while\\)\\>")
    (sh "{\\|\\<\\(case\\|for\\|if\\|until\\|while\\)\\>"))
  "Alist of regexps matching a block beginning.")

(defconst sh-end-block-re-alist
  '((csh "}\\|\\<\\(end\\|endif\\|endsw\\)\\>")
    (sh	"}\\|\\<\\(done\\|esac\\|fi\\)\\>"))
  "Alist of regexps matching a block end.")

(defconst sh-case-re-alist
  '((csh "\\<\\(case\\|default\\)\\>")
    (sh "[^()\n]+)"))
  "Alist of regexps matching a case statement entry.")

(defconst sh-else-re-alist
  '((csh "\\<else\\>")
    (sh "\\<\\(elif\\|else\\)\\>"))
  "Alist of regexps matching an else clause.")

(defvar sh-beg-block-re nil "Regexp matching a block beginning.")

(defvar sh-end-block-re nil "Regexp matching a block end.")

(defvar sh-case-re nil "Regexp matching a case statement entry.")

(defvar sh-else-re nil "Regexp matching an else clause.")

(defun sh-init-indent ()
  "Initialize variables for indentation."
  (set (make-local-variable 'sh-beg-block-re)
       (car (sh-feature sh-beg-block-re-alist)))
  (set (make-local-variable 'sh-end-block-re)
       (car (sh-feature sh-end-block-re-alist)))
  (set (make-local-variable 'sh-case-re)
       (car (sh-feature sh-case-re-alist)))
  (set (make-local-variable 'sh-else-re)
       (car (sh-feature sh-else-re-alist))))

(defconst sh-offsets-alist
  '((block-intro  . +)
    (block-close  . 0)
    (case-intro   . +)
    (case-close   . 0)
...
(defun sh-evaluate-offset (offset langelem symbol)
  "Offset can be a number, a function, a variable, a list, or one of
the symbols + or -."
  (cond
   ((eq offset '+)         (setq offset sh-indentation))
   ((eq offset '-)         (setq offset (- sh-indentation)))
   ((eq offset '++)        (setq offset (* 2 sh-indentation)))
   ((eq offset '--)        (setq offset (* 2 (- sh-indentation))))
   ((eq offset '*)         (setq offset (/ sh-indentation 2)))
   ((eq offset '/)         (setq offset (/ (- sh-indentation) 2)))
    ...
    (setq offset
	  (let (done)
	    (while (and (not done) offset)
	      (setq done (sh-evaluate-offset (car offset) langelem symbol)
		    offset (cdr offset)))
	    (if (not done)
		0
	      done))))
...
(defun sh-get-offset (langelem)
  "Get offset from LANGELEM which is a cons cell of the form:
\(SYMBOL . RELPOS).  The symbol is matched against
sh-offsets-alist and the offset found there is either returned,
or added to the indentation at RELPOS.  If RELPOS is nil, then
the offset is simply returned."
  (let* ((symbol (car langelem))
	 (relpos (cdr langelem))
	 (match  (assq symbol sh-offsets-alist))
	 (offset (cdr-safe match)))
    (if (not match)
	(setq offset 0
	      relpos 0)
      (setq offset (sh-evaluate-offset offset langelem symbol)))
    (+ (if (and relpos
	   ...
	   (save-excursion
	     ...
	     (current-column))
	 0)
       (sh-evaluate-offset offset langelem symbol))))

(defsubst sh-point (position)
...
(defun sh-in-literal (&optional lim)
  "Determine if point is in a literal."
  (save-excursion
    (let ((state (parse-partial-sexp (sh-point 'bol) (point))))
      (cond
       ((nth 3 state) 'string)
       ((nth 4 state) 'comment)
       (t nil)))))

(defun sh-in-comment-p ()
  "Determine if point is in a sh comment."
  (save-excursion
    (nth 4 (parse-partial-sexp (sh-point 'bol) (point)))))

(defun sh-skip-forward-literal ()
  "Skip forward literal and return t if within one."
  (let ((state (save-excursion (parse-partial-sexp (point-min) (point)))))
    (cond
     ((nth 3 state)			; inside string
      (skip-syntax-forward "^\"") (forward-char) t)
     ((nth 4 state) (forward-line 1) t)	; inside comment
     (t nil))))

(defun sh-skip-backward-literal ()
  "Skip backward literal and return t if within one."
  (let ((state (save-excursion (parse-partial-sexp (point-min) (point)))))
    (cond
     ((nth 3 state)			; inside string
      (skip-syntax-backward "^\"") (backward-char) t)
     ((nth 4 state) (search-backward "#") t) ; inside comment
     (t nil))))

(defsubst sh-re-search-forward (regexp &optional bound noerror)
  ...
  (while (and (re-search-forward regexp bound noerror)
	      (sh-skip-forward-literal)
	      (progn (store-match-data '(nil nil))
		     (if bound (< (point) bound) t))))
  (match-end 0))

(defsubst sh-re-search-backward (regexp &optional bound noerror)
  "Like `re-search-backward', but skips over matches in literals."
  (store-match-data '(nil nil))
  (while (and (re-search-backward regexp bound noerror)
	      (sh-skip-backward-literal)
	      (progn (store-match-data '(nil nil))
		     (if bound (> (point) bound) t))))
  (match-end 0))

(defun sh-beg-of-command ()
  "Go to beginning of current command."
  (beginning-of-line)
  (let (pos)
    (while (save-excursion (sh-backward-syntactic-ws)
			   (setq pos (point))
			   (= (preceding-char) ?\\))
      (goto-char pos)
      (beginning-of-line))))

(defun sh-forward-syntactic-ws (&optional lim)
...
(defun sh-backward-syntactic-ws (&optional lim)
...
(defmacro sh-add-syntax (symbol &optional relpos)
  "A simple macro to append the syntax in symbol to the syntax list.
try to increase performance by using this macro."
  `(setq syntax (cons (cons ,symbol ,relpos) syntax)))

(defun sh-corresponding-begin ()
  "Find corresponding block begin if cursor is at a block end."
  (while (and (sh-re-search-backward
	       (concat "\\(}\\|^\\s-*\\(" sh-end-block-re
		       "\\)\\)\\|{\\|^\\s-*\\(" sh-beg-block-re "\\)")
	       nil t)
	      (match-string 1))
    (sh-corresponding-begin)))

(defun sh-previous-case ()
  "Find previous case if cursor is at a case."
  (while (and (sh-re-search-backward
	       (concat "\\(}\\|^\\s-*\\(" sh-end-block-re
		       "\\)\\)\\|{\\|^\\s-*\\(" sh-beg-block-re
		       "\\)\\|^\\s-*\\(" sh-case-re "\\)")
...
(defun sh-guess-basic-syntax ()
  ...
  (save-excursion
    ...
    (let (syntax placeholder)
      (cond
       ;; else clause
       ((save-excursion (back-to-indentation) (looking-at sh-else-re))
	(sh-corresponding-begin)
	(sh-add-syntax 'else-clause (sh-point 'boi)))
       ;; block closing
       ((save-excursion (back-to-indentation) (looking-at sh-end-block-re))
	(sh-corresponding-begin)
	(sh-add-syntax 'block-close (sh-point 'boi)))
       ;; block opening
       ((save-excursion (sh-backward-syntactic-ws)
			(setq placeholder (point))
			(skip-syntax-backward "(")
			(or (looking-at sh-beg-block-re)
			    (progn (back-to-indentation)
				   (looking-at (concat sh-else-re "\\|"
						       sh-beg-block-re)))))
	(goto-char placeholder)
	(sh-add-syntax 'block-intro (sh-point 'boi)))
       ;; case closing
       ((save-excursion (back-to-indentation) (looking-at sh-case-re))
	(sh-previous-case)
	(sh-add-syntax 'case-close (sh-point 'boi)))
       ;; case opening
       ((save-excursion (sh-backward-syntactic-ws)
			(back-to-indentation)
			(when (looking-at sh-case-re)
			  (setq placeholder (point))))
	(goto-char placeholder)
	(sh-add-syntax 'case-intro (sh-point 'boi)))
       ;; command continuation
       ((save-excursion (sh-backward-syntactic-ws)
			(= (preceding-char) ?\\))
	(sh-beg-of-command)
	(sh-add-syntax 'command-cont (sh-point 'boi)))
       ;; new command
       (t
	(beginning-of-line)
	(sh-backward-syntactic-ws)
	(sh-beg-of-command)
	(sh-add-syntax 'command (sh-point 'boi))))
      syntax)))

(defun sh-indent-line ()
  "Indent the current line as sh code. Optional SYNTAX is the
syntactic information for the current line. Returns the amount of
indentation change (in columns)."
  (interactive)
  (let* ((syntax (sh-guess-basic-syntax))
	 (pos (- (point-max) (point)))
	 (indent (apply '+ (mapcar 'sh-get-offset syntax)))
	 (shift-amt  (- (current-indentation) indent)))
    (unless (zerop shift-amt)
      (beginning-of-line)
      (delete-region (point) (sh-point 'boi))
      (indent-to indent))
;    (message "syntax: %s, indent: %s" syntax indent)
    (if (< (point) (sh-point 'boi))
	(back-to-indentation)
...
(defun sh-indent-buffer ()
  "Indent whole buffer as sh code.
Calls `indent-region' for whole buffer."
  (interactive)
  (indent-region (point-min) (point-max) nil))

(defun sh-indent-region (start end column)
  "Indent region as sh code."
  (interactive "r\nP")
  (indent-region start end column))

(defsubst sh-indent-block-closing ()
  "If previous word is a block closing or `else', indent line again."
  (when (= (char-syntax (preceding-char)) ?w)
    (save-excursion
      (backward-word 1)
      (when (and (not (sh-in-literal))
		 (looking-at (concat sh-end-block-re "\\|" sh-else-re
				     "\\|" sh-case-re)))
	(sh-indent-line)))))

;; electrifications
(defun sh-electric-tab (&optional prefix-arg)
...
toggle between normal indent and inline comment indent,
  ...
  (cond ((> (current-column) (current-indentation))
	 (tab-to-tab-stop))
	((and (or (eq last-command 'sh-electric-tab)
		  (eq last-command 'sh-electric-return))
	      (/= 0 (current-indentation)))
	 (backward-delete-char-untabify sh-indentation nil))
	(t (sh-indent-line)))
  (setq this-command 'sh-electric-tab))

(defun sh-electric-return ()
  "Insert newline and indent."
  (interactive)
  (sh-indent-block-closing)
  (newline-and-indent))

(defun sh-electric-space (arg)
...
(defun sh-electric-closing-brace ()
...
;; commenting
(defun sh-comment-uncomment-region (beg end &optional arg)
