...
;; Copyright (C) 1995-1998 Tomas Abrahamsson <tab@lysator.liu.se>

;; Author:       Tomas Abrahamsson <tab@lysator.liu.se>
;; Maintainer:   Tomas Abrahamsson <tab@lysator.liu.se>
;; Keywords:     mouse
;; Version:	 1.1-beta1
;; Release-date: 21-Apr-1998
;; Location:     http://www.lysator.liu.se/~tab/artist/
;; Id:           $Id: artist.el,v 1.36 1998/04/20 23:24:46 tab Exp $
...
;; Artist is developed and tested with Emacs 19.34. It seams to
;; work with Emacsen 19.28 or higher (although the drawing won't work
;; with the shift-key in 19.29 and 19.30). It has been tested in Emacs
;; 20.2 (which was built with Erik Naggum's Multibyte Survival Kit,
...
;, New:		Spray-can (Utterly uselesse, I believe, but it was fun
...
(defvar artist-rubber-banding t
  "*Non-`nil' means interactively do rubber-banding.")


(defvar artist-first-char ?1
  "*Character to set at first point when not rubber-banding.")


(defvar artist-second-char ?2
  "*Character to set at second point when not rubber-banding.")


(defvar artist-interface-with-rect t
...
(defvar artist-arrows [ ?> nil ?v ?L ?< nil ?^ nil ]
  "*This is a vector of characters to use as arrows.  The vector is 8
elements long and contains a character for each direction, or nil if
there is no suitable character to use for arrow in that direction.
...
(defvar artist-aspect-ratio 1
  "*Character height-to-width aspect ratio, used when drawing squares
and circles. If the height of the characters on your terminal are 1.5
times the width, then setting `artist-aspect-ratio' to 1.5 will make
squares look square and circles look circular, but on another terminal
they might look different. If `artist-aspect-ratio' is 1, then squares
and circles will be as tall as they are wide, measured in characters.")


(defvar artist-trim-line-endings t
  "*If non-`nil', line-endings are trimmed (that is, extraneous
white-space at the end of the line is removed) when the shape is drawn.")


(defvar artist-flood-fill-right-border 'window-width

  "*When flood-filling, if the area is not closed to the right, then
flood-filling will fill no more to the right than specified by this
variable. This limit is called the fill-border.
Value can be:
  a number        This column is the fill-border.
  `window-width'  The fill-border is given by the window size.
  `fill-column'   The fill-border is given by the value of the
                  variable `fill-column'.")


(defvar artist-flood-fill-show-incrementally t
...
(defvar artist-ellipse-right-char ?\)
...
(defvar artist-ellipse-left-char ?\(
...
(defvar artist-picture-compatibility t
  "*If non-`nil', picture mode compatibility is on.")


(defvar artist-vaporize-fuzziness 1
  "*Accept this many characters cutting off a line and still treat
it as one line.")


(defvar artist-text-renderer 'artist-figlet
  "*Function for doing text rendering.")


(defvar artist-figlet-program "figlet"
  "*Program to run for `figlet'.")


(defvar artist-figlet-default-font "standard"
  "*Default font for `figlet'.")


(defvar artist-figlet-list-fonts-command
  ...
  "*Figlet switch to get font directory.")


(defvar artist-spray-interval 0.2
  "*Number of seconds between repeated spraying.")


(defvar artist-spray-radius 4
  "*Size of the area for spraying.")


(defvar artist-spray-chars '(?\  ?. ?- ?+ ?m ?% ?* ?#)
  "*Characters (``color'' to use when spraying. They should be ordered
from the ``lightest'' to the ``darkest'' since spraying replaces a
light character with the next darker one.")


(defvar artist-spray-new-char ?.
  "*Use this as initial character if spraying upon a character that is
...
(defconst artist-version "1.1-beta1")
...
(defvar artist-borderless-shapes nil
  "When non-`nil', shapes are drawn without a line border, the fill-char
is used instead if it is set.")
...
There are three kinds of tags:
  ...
  separator		-- produce a separator in the pop-up menu
  menu			-- a sub-menu

This is the layout of the info-part for graphics-operation:

  ( \"<name for shifed and unshifted operation>\"       ; appears in popup-menu
    (
      ( ; Info for unshifed operation
        \"<keyword>\"   ; Name of shape (when selecting operation).
        <symbol>	; Symbol (used to access the members below).
        \"<mode-line>\"	; String of shape to put in mode-line when drawing.
        <arrow-pred>	; Function to call to find out if shape takes arrows.
        <arrow-set-fn>	; Function to call to set arrow endpoints.
        <init-fn>	; Function to call at when first point is set.
        <prep-fill-fn>	; Function to call after last point is set, but before
                        ; filling is done.
        <exit-fn>	; Function to call after filling is done.
        <draw-how>      ; artist-do-continously:   do operation continously
                        ;                           as long as button is down
                        ; artist-do-poly:          do operation many times
                        ; <a number>:              wait for this many
                        ;                           points to be set before
                        ;                           drawing shape. If number
                        ;                           is 2: do rubberbanding.
        <draw-fn>	; Function to call for drawing the shape
	; The rest of the layout depends on the value of <draw-how>:
        ;  When <draw-how> is `artist-do-continously':
	     <interval-fn>	; Function for calling to get an
				; integer that specifies the interval
				; (number of seconds) for repeated calls to
				; <draw-fn> even when the user doesn't move
				; mouse. Use `nil' for no calls to <draw-fn>
				; when the mouse is not moved.
				; For now, this is only used when drawing
				; with the mouse.
        ;  When <draw-how> is 1:
	;    No furthre elements
        ;  When <draw-how> is other than `artist-do-continously' or 1:
             <undraw-fn>	; Function to call for undrawing shape
             <fill-pred>	; Function to call to find out if we shall fill
             <fill-fn>		; Function to call for filling
      )
      (
        ; Here comes the same info for the shifted operation
      )
    )
  )
...
This is the layout of the info-part for function-call:
  ...
  (
    \"<keyword>\"	; Name of the operation (appears in popup-menu)
    <symbol>		; Symbol for operation
    <fn>		; Function for performing the operation
  )

The layout of the info-part for separator is empty and not used.

This is the layout of the info-part for menu:
  (
    \"Title\"		; The title of the sub-menu
    (
      (tag1 info-part1)	; First menu item
      (tag2 info-part2)	; Second menu item
      ...
    )
  )
...
(defun artist-mode-init ()
  "Init artist-mode. Calls `artist-mode-init-hook'."
  (let ((i 0))
    (while (< i 256)
...
(defun artist-mode-exit ()
  "Exit artist-mode. Calls `artist-mode-exit-hook'."
...
(defun artist-t-if-fill-char-set ()
  "Returns value of variable artist-fill-char-set."
...
(defun artist-compute-popup-menu-table-sub (menu-table)
  "Compute operation table suitable for 'x-popup-menu."
  (mapcar
   (lambda (element)
...
(defun artist-compute-key-compl-table (menu-table)
  "Compute operation completion table, suitable for `completing-read'."
  (apply
   'nconc
   (artist-remove-nulls
    (mapcar
...
(defun artist-go-retrieve-from-symbol (symbol retrieve-fn)
  "Search the master table for a graphics operation SYMBOL. Calls RETRIEVE-FN
to retrieve information from that symbol's info-variant-part."
...
(defun artist-go-retrieve-from-symbol-sub (table symbol retrieve-fn)
  "Search the TABLE for a graphics operation SYMBOL. Calls RETRIEVE-FN
to retrieve information from that symbol's info-variant-part."
  (catch 'found
    (mapcar
     (lambda (element)
...
(defun artist-go-get-symbol-shift (symbol is-shifted)
  "Search for (shifted or unshifted) graphics operation SYMBOL. If IS-SHIFTED
is non-`nil', return the shifted symbol, otherwise the shifted symbol."
  (artist-go-get-symbol-shift-sub artist-mt symbol is-shifted))

(defun artist-go-get-symbol-shift-sub (table symbol is-shifted)
  "Search TABLE for (shifted or unshifted) graphics SYMBOL. If IS-SHIFTED
is non-`nil', return the shifted symbol, otherwise the shifted symbol."
  (catch 'found
    (mapcar
     (lambda (element)
...
(defun artist-fc-retrieve-from-symbol (symbol retrieve-fn)
  "Search the master table for a function-call SYMBOL. Calls RETRIEVE-FN
to retrieve information from that symbol's info-variant-part."
...
(defun artist-fc-retrieve-from-symbol-sub (table symbol retrieve-fn)
  "Search TABLE for a function-call SYMBOL. Calls RETRIEVE-FN
to retrieve information from that symbol's info-variant-part."
  (catch 'found
    (mapcar
     (lambda (element)
...
(defmacro artist-funcall (fn &rest args)
  "Calls function FN with ARGS iff FN is not nil."
  (list 'if fn (cons 'funcall (cons fn args))))

(defun artist-butlast (l)
  "Returns the list with all elements but the last."
  (cond ((null l) nil)
	((null (cdr l)) nil)
...
(defsubst artist-current-column ()
  "Returns point's current column."
  (current-column))

(defsubst artist-current-line ()
  "Returns point's current line \(buffer-relative\). Top of buffer is 0."
  (+ (count-lines 1 (point))
     (if (= (current-column) 0) 1 0)
     -1))

(defsubst artist-move-to-xy (x y)
  "Moves to column X, at row Y from the top of buffer (top line i 0)."
  ;;
...
(defsubst artist-replace-char (new-char)
  "Replaces the character at point with NEW-CHAR."
...
(defsubst artist-replace-chars (new-char count)
  "Replaces characters at point with NEW-CHAR. COUNT chars are replaced."
  (let ((overwrite-mode 'overwrite-mode-textual)
	(fill-column 32765)		; Large :-)
	(blink-matching-paren nil))
    (setq last-command-event (aref artist-replacement-table new-char))
...
(defsubst artist-replace-string (string &optional see-thru)
  "Replaces contents at point with STRING. With optional argument SEE-THRU,
set to non-`nil', text in the buffer ``shines thru'' blanks in the STRING."
  (let ((char-list (append string nil))	; convert the string to a list
	...
	(if (and see-thru (= (aref artist-replacement-table c) ?\ ))
	    (artist-move-to-xy (1+ (artist-current-column))
			       (artist-current-line))
	  (progn
	    (setq last-command-event (aref artist-replacement-table c))
	    (self-insert-command 1))))
...
(defun artist-no-rb-set-point1 (x y)
  "Sets point 1 at X, Y when not rubber-banding."
  (let ((x-now (artist-current-column))
	(y-now (artist-current-line)))
...
(defun artist-no-rb-unset-point2 ()
  "Unsets point 2 when not rubber-banding."
...
(defun artist-no-rb-set-point2 (x y)
  "Sets point 2 at X, Y when not rubber-banding."
  (let ((x-now (artist-current-column))
	(y-now (artist-current-line)))
...
(defun artist-no-rb-unset-points ()
  "Unsets point 1 and 2 when not rubber-banding."
...
(defvar artist-direction-info
  ...
  [ [  1  0 ?- ]			; direction 0
    ...
    [  0 -1 ?| ]			; direction 6
    [  1 -1 ?/ ] ]			; direction 7
  "Table used for stepping x and y coordinates in a specific direction and
for determining which char to use for that direction.")

(defsubst artist-direction-step-x (direction)
  "Returns the x-step in the artist-direction-info table."
  (aref (aref artist-direction-info direction) 0))

(defsubst artist-direction-step-y (direction)
  "Returns the y-step in the artist-direction-info table."
  (aref (aref artist-direction-info direction) 1))

(defun artist-direction-char (direction)
  "Returns the character in the artist-direction-info table."
...
(defun artist-find-direction (x1 y1 x2 y2)
  "Finds the direction when from point X1,Y1 to X2,Y2.
Direction is a number, 0--7 coded as follows:
...
(defun artist-straight-calculate-length (direction x1 y1 x2 y2)
  "Calculates length for a straight line in DIRECTION from X1,Y1 to X2,Y2."
  (cond ((or (= direction 7)
	     (= direction 0)
	     (= direction 1)) (1+ (- x2 x1)))
	((or (= direction 3)
...
(defun artist-save-chars-under-sline (line)
  "Saves characters under a STRAIGHT-LINE."
  (let ((x (aref line 0))
       	(y (aref line 1))
...
(defun artist-find-octant (x1 y1 x2 y2)
  "Finds octant for a line from X1,Y1 to X2,Y2.
...
(defsubst artist-new-coord (x y &optional new-char)
  "Creates a new coordinate at X,Y for use in a line. Optional arg: NEW-CHAR."
...
;; Calculate list of points using enght point algorithm
;; return a list of coords
;;
(defun artist-eight-point (x1 y1 x2 y2)
  "Runs the eight-point algorithm to get a coord list from X1,Y1 to X2,Y2."
  (let* ((point-list nil)
...
(defun artist-save-chars-under-point-list (point-list)
  "Saves characters originally under POINT-LIST."
  (mapcar
   (lambda (coord)
...
(defun artist-calculate-new-char (last-coord new-coord)
  "Returns which line-char to use when moving from COORD-1 to COORD-2."
...
(defun artist-calculate-new-chars (point-list)
  "Returns point-list with line-chars calculated. Input: POINT-LIST."
...
(defun artist-modify-new-chars (point-list)
  "Replace intersecting characters in POINT-LIST. Returns point-list."
  (mapcar
   (lambda (coord)
...
Output is a line (which is currently a list consisting of
  Endpoint1: a vector [x1 y1]
  Endpoint2: a vector [x2 y2]
  Shapeinfo: a list of vectors: [x y saved-char new-char])."
...
(defun artist-draw-sline (x1 y1 x2 y2)
  "Draws a strait line from X1, Y1 to X2, Y2. Straight lines are
vertical, horizontal or diagonal lines. They are faster to draw
and most often they are what you need when drawing a simple image.

Output is a straight line (which is currently a list consisting of:
  Endpoint1: a vector [x1 y1]
  Endpoint2: a vector [x2 y2]
  Shapeinfo: a vector [start-x start-y
		       length-of-line direction orig-char_1 ... ])."
...
Output is a rectangle (which is currently a list consisting of
  Endpoint1: a vector [x1 y1]
  Endpoint2: a vector [x2 y2]
  Shapeinfo: a list of four lines)."
...
(defun artist-rect-corners-squarify (x1 y1 x2 y2)
  "Takes rectangle corners X1, Y1 and X2, Y2 and computes corners for
a square with first corner at X1, Y1. The position of the second corner
depends on which of X2 and Y2 is most distant from X1, Y1."
  (let* ((delta-x      (- x2 x1))
	 (delta-y      (- y2 y1))
...
(defun artist-draw-square (x1 y1 x2 y2)
  "Draws a square with (rectangle) corners first corner at
X1, Y1 and X2, Y2.
Output is a square (which is currently a list consisting of
  Endpoint1: a vector [x1 y1]
  Endpoint2: a vector [x2 y2]
  Shapeinfo: a list of four lines)."
  (let* ((artist-line-char (artist-compute-line-char))
...
(defun artist-fill-rect (rect x1 y1 x2 y2)
  "Fills a RECTANGLE from X1, Y1, X2, Y2."
  (let ((x (1+ (min x1 x2)))
	(y (1+ (min y1 y2)))
	(x-max (max x1 x2))
	(y-max (max y1 y2)))
...
(defun artist-fill-square (square x1 y1 x2 y2)
  "Fills a SQUARE from X1, Y1, X2, Y2."
...
(defun artist-text-insert-common (x y text see-thru)
  "At position X, Y, insert text TEXT. If SEE-THRU is non-`nil', then
blanks in TEXT does not replace text in the buffer."
...
(defun artist-text-insert-see-thru (x y text)
  "At position X, Y, insert text TEXT. Let text already in buffer shine
thru the TEXT inserted."
  (artist-text-insert-common x y text t))

(defun artist-text-insert-overwrite (x y text)
  "At position X, Y, insert text TEXT. Let blanks in TEXT overwrite any
text already in the buffer."
  (artist-text-insert-common x y text nil))

(defun artist-text-see-thru (x y)
  "Prompt for text to render, render it at X,Y by calling
the function specified by `artist-text-renderer', which must return
a list of strings, to be inserted in the buffer.
...
(defun artist-erase-rect (rect x1 y1 x2 y2)
  "Erases a RECTANGLE from X1, Y1, X2, Y2."
...
(defun artist-vap-find-endpoint (x1 y1 step-x step-y accept-set reject-set)
  "Find one endpoint for line through X1, Y1, stepping in direction
 STEP-X, STEP-Y, accepting characters in the list ACCEPT-SET, stopping
 immediately when finding characters in the list REJECT-SET
Fuzziness (that is, the number of consecutive characters not in ACCEPT-SET
 to allow as part of the line) is determined by the variable
 `artist-vaporize-fuzziness'.
...
(defun artist-vaporize-line (x1 y1)
  "Vaporizes (erases) the straight line through X1, Y1 by
replacing the characters that forms the line with `artist-erase-char'.
Output is a list of endpoints for lines through X1, Y1.
An endpoint is a cons pair \(ENDPOINT-X . ENDPOINT-Y\)."
...
(defun artist-ellipse-generate-quadrant (x-radius y-radius)
  "Creates a point-list for first quadrant. Points go from (0, y) to (x, 0).
...
(defsubst artist-new-fill-item (x y width)
  "Creates a new item at X, Y, with WIDTH for use in fill-info
in ellipses and circles."
...
(defun artist-ellipse-compute-fill-info (point-list)
  "Compute fill-info for ellipse around 0,0 from POINT-LIST which covers the
first quadrant."
  (let ((first-half nil)
	(both-halves nil)
	(last-y nil))
...
(defun artist-draw-ellipse-general (x y x-radius y-radius)
  "Draws an ellipse with center at X, Y and X-RADIUS and Y-RADIUS.
Output is an ellipse (which is currently a list consisting of
  Endpoint1: a vector [x1 y1]
  Endpoint2: a vector [x2 y2]
  Shapeinfo: a two-element vector: [point-list fill-info] where
      point-list: a list of vectors [x y saved-char new-char]
      fill-info:  a list of vectors [x y width-of-ellipse-on-this-line]).
...
(defun artist-draw-ellipse (x1 y1 x2 y2)
  "Draws an ellipse with center at X1, Y1 and point X2,Y2.
Output is an ellipse (which is currently a list consisting of
  Endpoint1: a vector [x1 y1]
  Endpoint2: a vector [x2 y2]
  Shapeinfo: a two-element vector: [point-list fill-info] where
...
(defun artist-draw-circle (x1 y1 x2 y2)
  "Draws a circle with center at X1, Y1 and point X2,Y2.
Output is an ellipse (which is currently a list consisting of
  Endpoint1: a vector [x1 y1]
  Endpoint2: a vector [x2 y2]
  Shapeinfo: a two-element vector: [point-list fill-info] where
...
(defun artist-fill-ellipse (ellipse x y x-radius y-radius)
  "Fills an ELLIPSE centered at X,Y with radius X-RADIUS and Y-RADIUS."
...
(defun artist-cut-rect (rect x1 y1 x2 y2)
  "Copies a RECTANGLE drawn from X1, Y1 to X2, Y2, then clears it."
...
(defun artist-cut-square (square x1 y1 x2 y2)
  "Copies a SQUARE drawn from X1, Y1 to X2, Y2 (made square), then clears it."
...
(defun artist-copy-generic (x1 y1 x2 y2)
  "Copies rectangular area with corners at X1, Y1 and X2, Y2.
Output is a copy buffer (which is currently a list of strings)
representing the original contents of that area in the buffer."
  (let* ((x    	(min x1 x2))
	 (y    	(min y1 y2))
	 (x-max (max x1 x2))
	 (y-max (max y1 y2))
...
(defun artist-copy-rect (rect x1 y1 x2 y2)
  "Copies a RECTANGLE drawn from X1, Y1 to X2, Y2."
  (artist-undraw-rect rect)
  (artist-copy-generic x1 y1 x2 y2))
...
(defun artist-make-arrow-point (x y direction &optional state)
  "Creates an arrow-point at X, Y for a line in direction DIRECTION.
...
(defun artist-set-arrow-points-for-2points (shape x1 y1 x2 y2)
  "Generic function for setting arrow-points for 2-point shapes from SHAPE
...
(defun artist-draw-region-reset ()
  "Resets the current draw-region."
  (setq artist-draw-region-max-y 0)
  (setq artist-draw-region-min-y line-number-display-limit))

(defun artist-draw-region-trim-line-endings (min-y max-y)
  "Trims (removes-white-space at end of line) line-endings in current
draw-region."
...
(defun artist-key-do-continously-continously (x y)
  "Updates current continous shape at X,Y."
  (let ((draw-fn   (artist-go-get-draw-fn-from-symbol artist-curr-go)))
    (artist-funcall draw-fn x y)))


(defun artist-key-do-continously-poly (x y)
  "Updates current poly-point shape with nth point at X,Y."
  (let ((draw-fn   (artist-go-get-draw-fn-from-symbol artist-curr-go))
...
(defun artist-key-set-point-continously (x y)
  "Sets point for current continous shape at X,Y."
...
(defun artist-key-set-point-poly (x y &optional this-is-last-point)
  "Sets point for current poly-point shape at X,Y.
...
(defun artist-key-set-point-1point (x y)
  "Sets point for current 1-point shape at X,Y."
  (let ((draw-fn      (artist-go-get-draw-fn-from-symbol artist-curr-go))
	(init-fn      (artist-go-get-init-fn-from-symbol artist-curr-go))
	(prep-fill-fn (artist-go-get-prep-fill-fn-from-symbol artist-curr-go))
	(exit-fn      (artist-go-get-exit-fn-from-symbol artist-curr-go))
...
With arg, turn rubber-banding on iff arg is positive."
  (interactive)
  (if artist-key-is-drawing
...
(defun artist-toggle-trim-line-endings (&optional state)
  "Toggle trimming of line-endings. With arg, turn on iff arg is positive."
  (interactive)
...
(defun artist-toggle-borderless-shapes (&optional state)
  "Toggle borders of shapes. With arg, turn on iff arg is positive."
  (interactive)
...
(defsubst artist-event-is-shifted (ev)
  "Returns non-`nil' if the shift-key is pressed in EVENT."
...
(defun artist-down-mouse-3 (ev)
  "Erase character or rectangle, depending on EVENT."
  (interactive "@e")
...
(defun artist-mouse-draw-continously (ev)
  "Generic function for shapes requiring 1 point (current position) as input.
...
;;    prep-fill-fn, arrow-fn and exit-fn are stated.
;;
