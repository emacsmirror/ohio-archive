...
;; Copyright 1997, 1998 by Bob Glickstein  <http://www.zanshin.com/~bobg/>
...
backreference numbers to numbers suitable for the generated regex.
...
(defun sregex-replace-match (replacement
  ...
  "Like `replace-match' when the regex used came from sregex.
...
(defun sregex-match-string (count &optional in-string sregex)
  "Like `match-string' when the regex used came from sregex.
This takes one additional optional argument, the sregex info, which
...
(defun sregex-match-string-no-properties (count &optional in-string sregex)
  "Like `match-string-no-properties' when the regex used came from sregex.
This takes one additional optional argument, the sregex info, which
can be obtained with `sregex-info'.  The COUNT argument is adjusted to
allow for \"introduced groups\".  If the extra argument is omitted or
nil, it defaults to the current value of (sregex-info)."
...
(defun sregex-match-beginning (count &optional sregex)
  "Like `match-beginning' when the regex used came from sregex.
This takes one additional optional argument, the sregex info, which
can be obtained with `sregex-info'.  The COUNT argument is adjusted to
allow for \"introduced groups\".  If the extra argument is omitted or
nil, it defaults to the current value of (sregex-info)."
...
(defun sregex-match-end (count &optional sregex)
  "Like `match-end' when the regex used came from sregex.
This takes one additional optional argument, the sregex info, which
can be obtained with `sregex-info'.  The COUNT argument is adjusted to
allow for \"introduced groups\".  If the extra argument is omitted or
nil, it defaults to the current value of (sregex-info)."
...
(defun sregex-match-data (&optional sregex)
  "Like `match-data' when the regex used came from sregex.
This takes one additional optional argument, the sregex info, which
...
(defmacro sregexq (&rest exps)
  "Symbolic regular expression interpreter.
This macro allows you to specify a regular expression (regex) in
symbolic form and converts it into the string form required by Emacs'
regex functions such as `re-search-forward' and `looking-at'.  Here's
a simple example:
