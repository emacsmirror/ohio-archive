...
;; Keywords:   data, ascii
;; Time-stamp: <2000/06/29 22:16:07 vinicius>
;; Version:    1.7
...
;;
;; `ascii-window-size'			Initial ASCII window size.
;;
;; `ascii-display-code'			List of character range to be
;;					displayed.
;;
;; `ascii-keep-window'			Non-nil means to keep ASCII window
;;					active.
;;
;; `ascii-table-separator'		String used to separate ASCII table
;;					columns.
;;
;; `ascii-ascii-face'			Symbol face used to highlight ascii
;;					code.
;;
;; `ascii-non-ascii-face'		Symbol face used to highlight non-ascii
;;					code.
;;
...
(defcustom ascii-window-size 6
  "*Initial ASCII window size."
  :type 'integer
  :group 'ascii)


(defcustom ascii-display-code '((?\000 . ?\377))
  "*List of character range to be displayed.
...
(defcustom ascii-table-separator "|"
  "*String used to separate ASCII table columns."
  :type 'string
  :group 'ascii)


(defcustom ascii-ascii-face 'ascii-ascii-face
  "*Symbol face used to highlight ascii code."
  :type 'face
  :group 'ascii)
...
(defcustom ascii-non-ascii-face 'ascii-non-ascii-face
  "*Symbol face used to highlight non-ascii code."
  :type 'face
  :group 'ascii)
...
(defun ascii-off ()
  ...
  (if (> ascii-reference-count 0)
    ...
    (remove-hook 'post-command-hook 'ascii-post-command)
...
(defun ascii-post-command ()
  (let* ((char (following-char))
    ...
    (cond ((and (boundp 'ascii-display)
	   ...
	   (ascii-show-table code (< char 256)))
...
(defun ascii-show-table (code ascii-p)
  (let ((buffer (ascii-get-buffer code)))
    (and
     ...
     ;; adjust overlay
     (let ((window     (get-buffer-window ascii-buffer-name))
