...
;;
;; The latest version of uptimes.el can be found at either of:
;;
;;   <URL:http://www.hagbard.demon.co.uk/archives/uptimes.el>
;;   <URL:http://www.acemake.com/hagbard/archives/uptimes.el>
...
;;
;; $Log: uptimes.el,v $
;; Revision 2.1  1999/06/24 12:05:55  davep
;; Added `uptimes-version'.
;; Added `uptimes-wordy-uptime'.
;; Re-wrote the body of `uptimes-update' so that there is less redundant code.
;; `uptimes-this' now reports `uptimes-wordy-uptime', before it simply showed
;; the elapsed time.
;;
;; Revision 2.0  1999/06/07 15:08:00  davep
;; Partial re-write. This revision eliminates any run-time reliance on the cl
;; package. This includes changing the basic structure of the alists.
;; Previously they were lists of structs, now they are "simply" lists.
;;
;; Revision 1.6  1999/06/07 09:23:21  davep
;; The "database" is now saved using `pp' to format the list.
;;
;; Revision 1.5  1999/05/28 12:32:39  davep
;; Added a `kill-emacs-hook' to ensure that the emacs always saves its uptime.
;;
;; Revision 1.4  1999/05/28 12:27:05  davep
;; Minor corrections to various comments and doc-strings.
;;
;; Revision 1.3  1999/05/28 11:48:18  davep
;; Remove the reliance on midnight.el for the time handling functions.
;; Added `uptimes-float-time' and `uptimes-time-float' to aid the above change.
;; Changed the default for `uptimes-auto-save-interval' from 1 second to 300
;; seconds.
;;
;; Revision 1.2  1999/05/27 14:09:25  davep
;; Modified `uptimes-uptimes' to use `with-output-to-temp-buffer' so that it
;; works more like many other "pop some output into a temporary buffer for
;; viewing" functions in emacs.
;; Renamed `uptimes-insert-uptimes' to `uptimes-print-uptimes' to reflect the
;; fact that it had to be changed to make the above modification work.
;;
;; Revision 1.1  1999/05/27 11:18:36  davep
;; Initial revision
;;
