...
;; Time-stamp:	<2000/06/28 09:55:31 vinicius>
;; Version:	1.3
...
;;
;; `highline-face'			Face used to highlight the current
;;					line.
;;
;; `highline-whole-line'		Non-nil means mark up to end-of-line;
;;					otherwise, mark up to window-border.
;;
;; `highline-verbose'			Non-nil means generate messages.
;;
;; `highline-ignore-regexp'		Regexp for buffers to ignore.
;;
...
(defcustom highline-whole-line t
  "*Non-nil means mark up to end-of-line; otherwise, mark up to window-border."
  :type 'boolean
  :group 'highline)
...
Path: news.cis.ohio-state.edu!news.ems.psu.edu!newsfeed.stanford.edu!skynet.be!newspump.monmouth.com!newspeer.monmouth.com!uunet!ffx.uu.net!spool1.news.uu.net!wendy-fate.uu.net!gnu-emacs-sources
Message-ID: <200007290129.WAA26230@java.cpqd.com.br>
From: vinicius@cpqd.com.br (Vinicius Jose Latorre)
Newsgroups: gnu.emacs.sources
Subject: highline.el v1.5
Date: Fri, 28 Jul 2000 22:29:37 -0300
Lines: 541
x-uunet-gateway: chi6sosrv11.alter.net from gnu-emacs-sources to gnu.emacs.sources; Sat, 29 Jul 2000 01:32:52 GMT
...
Xref: neutral.verbum.org gnu.emacs.sources:434
...
;; Maintainer:	Vinicius Jose Latorre <vinicius@cpqd.com.br>
;; Keywords:	faces, frames, editing
;; Time-stamp:	<2000/07/27 13:55:01 vinicius>
;; Version:	1.5
...
;; `highline-ignore-regexp'		Regexp for buffers to ignore.
;;
;; `highline-priority'			Overlay priority.
;;
;; To set the above options you may:
;;
;; a) insert the code in your ~/.emacs, like:
...
(or (fboundp 'line-end-position)
	...
	(and n (/= n 1) (forward-line (1- n)))
	(end-of-line)
	(if (and (eobp) (not highline-whole-line))
	    (1- (point))
	  (point)))))
...
(defsubst highline-p ()
  (or (and (symbolp 'highline-mode) highline-mode)
      (and (symbolp 'highline-local-mode) highline-local-mode)))
