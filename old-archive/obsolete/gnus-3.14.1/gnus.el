...
;; How to Install GNUS:
;; (0) First of all, remove GNUS related OLD *.elc files (at least nntp.elc).
;; (1) Unshar gnus.el, gnuspost.el, gnusmail.el, gnusmisc.el, and nntp.el.
;; (2) byte-compile-file nntp.el, gnus.el, gnuspost.el, gnusmail.el, and gnusmisc.el.
;;     If you have a local news spool, byte-compile-file nnspool.el, too.
...
(autoload 'gnus-Subject-mail-forward "gnusmail"
...
(if gnus-Subject-mode-map
  ...
  (define-key gnus-Subject-mode-map "\C-c\C-f" 'gnus-Subject-mail-forward)
...
(defun gnus-make-headers-hashtable-by-number ()
  ...
  (let ((header nil)
    ...
    (setq gnus-newsgroup-headers-hashtb-by-number (gnus-make-hashtable))
    (while headers
...
(defun gnus-make-headers-hashtable-by-id ()
  ...
  (let ((header nil)
	(headers gnus-newsgroup-headers))
    (setq gnus-newsgroup-headers-hashtb-by-id (gnus-make-hashtable))
    (while headers
      (setq header (car headers))
