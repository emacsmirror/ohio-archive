...
;;; version 1.0  (older versions weren't numbered!)

;;; LCD Archive Entry:
;;; bibtex-mode+|Marc Shapiro|shapiro@corto.inria.fr|
;;; Support for maintaining BibTeX format bibliography databases|
;;; 92-04-01|version 1.0|~/modes/bibtex-mode+.el.Z|
...
;;;   2. bibtex-next-field doesn't work properly when called from the
;;;      entry key position, it moves to the *second* field, not the first. 
...
(defvar bibtex-mode-map
  (let ((km (make-sparse-keymap)))
    ...
    (define-key km "\C-c\C-n" 'narrow-to-bibtex-entry)
...
(defun bibtex-entry (entry-type &optional required optional)
  ...
  (if (and (null required) (null optional))
      (let* ((e (assoc entry-type bibtex-entry-field-alist))
	     (r-n-o (elt e 1))
	     (c-ref (elt e 2)))
	(if (null e)
