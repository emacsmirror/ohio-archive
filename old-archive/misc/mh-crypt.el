...
;;;
;;; LCD Archive Entry:
;;; mh-crypt|Fritz Knabe|knabe@ecrc.de|
;;; Store and retrieve MH messages in encrypted or compressed form.|
;;; 14-Jun-1994|1.18|~/misc/mh-crypt.el.Z|
;;;
...
(defun mhc-enc-folder (encrypt range)
  ...
  (if (or range mh-do-not-confirm
      ...
      (progn
        ...
        (mh-seq-from-command mh-current-folder 'all
  ...
  (let* ((local-mhc-move-headers mhc-move-headers)
	...
	(if mhc-auto-crypt
	    (save-excursion
	      ...
	      (setq mhc-decrypted (if encrypt " Encrypt" " Encoded")))
	  (save-excursion
...
(defun mhc-dec-buffer (msg decrypt key-or-type guess)
  (let (mhc-auto-crypt decrypt-okay guesses)
    ...
    (save-excursion
	  ...
	  (progn
	    (goto-char (point-min))
	    (setq mhc-decrypted (if decrypt " Encrypt" " Encoded"))
	    (set-buffer-modified-p nil))))
