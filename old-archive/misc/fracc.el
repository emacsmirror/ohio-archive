;;; $Id: fracc.el,v 1.18 1994/12/18 16:29:41 queinnec Exp $
...
;;; 18-Dec-1994|1.18|~/misc/fracc.el.Z|
                      ...
                      (let ((md (match-data))
                        ...
                        (while (consp action)
                          (if (stringp (car action))
                              (fracc-replace-match (car action) md cs)
                            (eval (car action)) )
                          (setq action (cdr action)) )
                        (if (stringp action)
                            (fracc-replace-match action md cs)
...
(defun fracc-replace-match (regexp md cs)
...
(defvar fracc-html-encoding
  '(t ; = case-search-fold
    ...
    (?#
     ...
     ("u" . "&\\&uml;") )
    (?a
     ("c," "&\\&" (delete-backward-char 1) "cedil;") )
    (?u
     ("c," "&\\&" (delete-backward-char 1) "cedil;") )
    ;; oe does not exist.
    )
