...
;; Version:   2.31 (20 March 97)
;; Keywords:  bibtex, cite, auctex, emacs, xemacs

;; RCS $Id: bib-cite.el,v 1.12 1997/03/20 15:50:02 rhogee Exp $
...
;; 20-Mar-1997|2.31|~/misc/bib-cite.el.gz|
...
;;; (switch-to-buffer new-buffer)
    ...
    (cond
     ...
     ((featurep 'font-lock)             ;Perhaps let the user's setup determine
      (font-lock-fontify-buffer)))      ; if font-lock fontifies?
...
;;; Return a single list from a list that may contain either items
;;; or any number of list levels containing items.
;;; e.g. turns
...
(defun bib-cite-initialize ()
  ...
  (cond 
   ((and (string-match "XEmacs\\|Lucid" emacs-version)
	 (or (null (key-binding "\C-cb")) (keymapp (key-binding "\C-cb"))))
