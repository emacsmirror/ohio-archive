...
;; $modified: Fri Apr  1 15:15:39 1994 by rshouman $
;;
...
;; 01-April-1994|1.04|~/misc/search-completions.el.Z|
...
;; their names:
;; C-hvC-xC-nbuffer\RETC-xC-nwindow\RET\TAB
;;
;; To visit a file with a ".c" or ".h" extension:
;; C-xC-fC-n\.[ch]\RET\SPACE
;; 
...
(defvar isearch-completions-exit-char
  (if (string-match "^19" emacs-version) 13 nil)
...
(defvar narrow-completions-enable t
...
(defconst completions-buffer-name
...
minibuffer contents are not changed. "
  ...
  (let ((currwin (selected-window))
	...
	(version-19p (not (null (string-match "^19" emacs-version))))
	found)
    ...
    (if (null compwin)
	nil
      (let ((search-exit-char (or isearch-completions-exit-char
				  version-19p
				  search-exit-char))
...
subdirectories also. This function should be called from the minibuffer. "
  ...
  (let ((narrow-completions-depth (1+ narrow-completions-depth))
	...
	(version-19p (string-match "^19" emacs-version)))
    ;; If it's programmed completion, assume filename completion.
    (if (symbolp minibuffer-completion-table)
	(progn
...
only file names matching REGEXP, or directories, with optional COMPLEMENT,
