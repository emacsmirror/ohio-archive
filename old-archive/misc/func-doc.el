...
;; Author:    Peter S. Galbraith <P_Galbraith@Qc.DFO.ca>
;; Created:   15 Jan 1996.
;; Version:   1.07 (09 August 96)
;; Keywords:  info, completion, commands, latex, perl, awk, texinfo

;; RCS $Id: func-doc.el,v 1.10 1996/08/09 15:52:33 rhogee Exp $
...
;; LCD Archive Entry:
;; func-doc|Peter Galbraith|Galbraith@mixing.qc.dfo.ca|
;; Get Info on command under point; completion for commands.|
;; 09-August-1996|1.07|~/misc/func-doc.el|
;;----------------------------------------------------------------------------
;;; Commentary:
...
;; Eventually, do full in-text completion
  ...
  (let* ((str-list (func-doc-word nil))
    ...
    (if (not common) (message "Sorry, no completion for %s" str)
      (setq 
       common 
       (if (or (string-match "^18\\." emacs-version) (boundp 'epoch::version))
           (completing-read "Command: " cmd-alist nil t common)
         (completing-read "Command: " cmd-alist nil t common 'func-doc-hist)))
