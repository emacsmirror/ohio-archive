...
;; 18-Aug-94|3.04|~/misc/face-lock.el.Z|
...
;; 3.03--3.04:
;; - Twigged with initialisation of `face-lock-face-attributes'.
;; - Made `face-lock-make-face' set bold/italic only if non-nil.

(require 'font-lock)
    ...
    (cond ((memq face-lock-display-type '(mono monochrome))
	  ...
	  (light-bg			; light colour background
	   '((font-lock-comment-face "Firebrick")
	     (font-lock-doc-string-face "RosyBrown")
	  ...
	  (t				; dark colour background
	   '((font-lock-comment-face "OrangeRed")
	     (font-lock-doc-string-face "LightSalmon")
...
attributes may be given and used in highlighting.  See `font-lock-keywords'.
...
See variables `face-lock-display-type' and `face-lock-background-mode'.
...
the face is also created; its value is the face name."
  (let* ((face (nth 0 face-attributes))
	...
	(and (funcall on-p face-name "Bold") (set-face-bold-p face t))
      (and (nth 3 face-attributes) (set-face-bold-p face t)))
    (if (funcall set-p face-name "Italic")
	(and (funcall on-p face-name "Italic") (set-face-italic-p face t))
      (and (nth 4 face-attributes) (set-face-italic-p face t)))
