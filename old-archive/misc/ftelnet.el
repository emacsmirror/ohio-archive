...
; Subject: ftelnet.el - alternative telnet interface for emacs
; Date: Sun, 12 Nov 95 15:16:48 CST
; 
...
;; $Id: ftelnet.el,v 1.6 1995/10/27 22:48:39 friedman Rel $

;; LCD Archive Entry:
;; ftelnet|Noah Friedman|friedman@gnu.ai.mit.edu|
;; remote login interface|
;; 12-Nov-1995|1.6|~/misc/ftelnet.el.gz|
    ...
    (cond
     ...
     (t
      ...
      (cond
       ((or (null port)
            (string= port "23"))
        (add-hook 'comint-output-filter-functions 'ftelnet-user-output-filter)))
...
(defun ftelnet-send-urgently (urgsym)
  (let ((s (cdr (assq urgsym ftelnet-urgent-alist))))
    (and s (comint-send-string nil s))))
