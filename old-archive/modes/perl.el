;From ark1!uakari.primate.wisc.edu!zaphod.mps.ohio-state.edu!usc!cs.utexas.edu!uunet!ksr!bill Fri Mar 23 09:38:09 1990
;Article 1345 of gnu.emacs:
;Xref: ark1 comp.lang.perl:813 gnu.emacs:1345
;Path: ark1!uakari.primate.wisc.edu!zaphod.mps.ohio-state.edu!usc!cs.utexas.edu!uunet!ksr!bill
;>From bill@ksr.com (Bill Mann)
;Newsgroups: comp.lang.perl,gnu.emacs
;Subject: perl mode for emacs
;Message-ID: <630@ksr.UUCP>
;Date: 20 Mar 90 18:37:15 GMT
;Sender: bill@ksr.UUCP
;Organization: Kendall Square Research Corp.
;Lines: 630
;
;There have been a number of requests for a gnu-emacs perl-mode posted
;recently.  Here is the one I've been using, adapted from c-mode.  Please
;see the comments after the copyleft notice below for details.  My address:
;  bill@ksr.com (ksr!bill@uunet.uu.net)    -- Bill Mann
...
(defun indent-perl-exp ()
  ...
  (interactive)
  (let* ((case-fold-search nil)
	 (bof (save-excursion (end-of-line 2) (perl-beginning-of-function)))
	 (oldpnt (point-marker))
	 eol last-point lsexp delta)
    (if (= (char-after bof) ?=)
	...
	(setq last-point (point-marker)))
      (setq lsexp bof)
      (beginning-of-line)
      (while (< (point) (marker-position last-point))
	(setq delta (perl-indent-line nil bof))
	(if (numberp delta)		; unquoted start-of-line?
	    (setq lsexp (point)))
	(end-of-line) 
	(setq eol (point))
	(if (nth 4 (parse-partial-sexp lsexp eol))
