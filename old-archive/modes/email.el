...
;;
;; LCD Archive Entry:
;; email|G. Dinesh Dutt|brat@htilbom.ernet.in|
;; a set of mail-mode extensions for easy cc, fcc and reference addition|
;; 20-May-1994|1.2|~/modes/email.el.Z|
;;
...
;; a specified set of people automatically and add a reference id to the
...
;; must not use mail-auto-archive-filename as it adds an FCC field automatically
;; on invoking the mail-mode and this prevents me from adding an FCC field.  Set
;; the value of that variable to email-auto-archive-filename instead.  Also, if 
...
;; (setq email-cc-alist
;;    '(("To"
;; 	("abc.com\\b"."guy1, fawkes2")
;; 	("^dinesh@"."^vishy"))))
;; 
;; (setq email-id-alist			;; The second element of the alist
;;    '(("To"			        ;; must be non-nil always. Setting this
;; 	("^abcdef".t)                  ;; to nil results in wrong behaviour.
;; 	("abc.com\\b".t)		
;; 	("^dinesh@".t))))
;;
;; See the documentation of variables for further customization. 
;;

;;; BUGS(Features i.e. :-)
;;
;; A default archive file (specified via the mail-auto-archive-file-name) can 
...
;;
;; revision 1.2	
;; date: 1994/05/20 07:04:17;  author: brat;  state: Exp;  lines: +52 -31
;; Corrected id format from "USERNAME- ID," to "USERNAME-ID, ".
;; Modifications to suit GNU standard.
;;
;; revision 1.1
;; date: 1994/05/05 06:07:38;  author: root;  state: Exp;
;; Initial revision.
;;
;; Thanks to B.Vishwanath for pushing me to write this package and for helping
;; me bounce some of my ideas against him and for testing it.
;;
...
(defvar email-fcc-alist nil
  "*If non-nil, value should be an alist that provides a header-FCC mapping. 
The alist is of the form
\((HEADER-NAME
  (REGEXP.FOLDER-NAME) ...
... ))
HEADER-NAME is one of the mail headers from the list `email-mail-headers-list'. 
REGEXP is a string which is the pattern to match for and FOLDER-NAME is the
name of the folder into which the message is to be saved.  Once a match is
found,further searches are discarded.  One cannot therefore, save to multiple
files.") 

(defvar email-cc-alist nil
  "*If non-nil, value indicates an alist that provides a header-CC mapping.
...
(defvar email-id-alist nil
  "*If non-nil, must be a alist that will say if an id must be added for a mail
The alist is of the form :
\((HEADER-NAME
  (REGEXP.t) ...
... ))
...
(defvar email-use-fcc nil
  "*Non-nil means, the message to be sent is processed for fcc before sending.")

(defvar email-use-cc nil
  "*Non-nil means, the message to be sent is processed for cc before sending.")

(defvar email-use-ids nil
  "*Non-nil means, all mail messages must be scanned for possibility of adding 
an id.")

(defvar email-id-format (concat (user-login-name) "-")
  "*If non-nil, specifies the format of the id being appended to the subject
of the mail.  Default is \"USERNAME-ID, \" where USERNAME is
`(user-login-name)' and ID is the number retrieved from the id file.")
...
(defvar email-expand-headers t
  "*If non-nil, `(email-expand-headers)' will expand the headers in the mail buffer itself.")
...
;; This is the main driver routine for the rest of the package.  Based on the
;; value of the email-use-* variables, the corresponding functions will be
;; called. 
...
function.  This function invokes sendmail-send-it as the final step."
  (let ((delimline)
	(mtmpbuf (generate-new-buffer " email temp"))
...
If that variable is non-nil and no match is found, the value is set to the
value of email-auto-archive-filename."
  (let (email-outbox)
...
The variable email-cc-alist is scanned for a match with the current mail
headers. If it succeeds, a cc field is added (or appended if a cc already
exists) to the mail headers."
...
This function retrieves the next id to be used from the .mailrc file.  The id 
line must be the first line of the file and must be of the form #<id>.  For eg.
#100." 
  (if (and email-id-alist (consp email-id-alist))
...
It picks up the next word in the line (word is terminated by a space, comma
or newline) and checks if thats present in the given alist.  The mainkey
variable gives the current mail header name."
