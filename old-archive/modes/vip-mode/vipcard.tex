% Quick Reference Card for VIP 4.3 under GNU Emacs version 18 on Unix systems
%**start of header
...
\centerline{by Aamod Sane, \version}
\centerline{original author Masahiko Sato}
\centerline{for VIP 4.3 under GNU Emacs version 18 on Unix systems}
...
\centerline{(for version 4.3 under GNU Emacs version 18)}
...
VIP has four modes: {\it emacs mode}, {\it vi mode}, {\it insert mode}, 
{\it replace mode}.
...
\metax{from vi mode to insert mode}{i, I, a, A, o, O}
\metax{from vi mode to replace mode}{c, C, R}
\metax{from replace mode to insert mode}{Cross \$ sign}
\key{from insert or replace mode to vi mode}{ESC}
...
\section{Insert Mode}
You can do some editing in insert mode.

\metax{go back to vi mode}{ESC, C-c}
...
\key{emulate Meta key in emacs mode}{C-z}
...
if you have enabled help.

\section{Leaving Emacs}

\metax{suspend Emacs}{:st {\rm or} :su}
\metax{exit Emacs permanently}{Z Z {\rm or} C-xC-c}
...
\key{abort partially typed or executing command}{C-c}
\key{redraw messed up screen}{C-l}
\metax{{\bf recover} a file lost by a system crash}{:rec file}
...
\metax{{\bf switch} to a buffer in another window}{:n {\rm or} C-x 4 {\sl buffer}}
...
\key{{\bf get} information on the current {\bf file}}{C-g {\rm or} :f}
...
\twocol{vip-filename-completion}{t}
\twocol{vip-always}{t}
...
See the VIP manual for details on these options.
To bind keys for Vi mode,

\beginexample
(define-key vip-mode-map "\\C-s" 'isearch-forward)
(define-key vip-mode-map "\\C-r" 'isearch-backward)
\endexample
...
Similiarly, \# is replaced by previous file. For VIP, this is the
...
\key{define a macro {\it x} that expands to {\sl cmd} in insert mode}{:map!\ {\it x} {\sl cmd}}
\key{remove macro expansion associated with {\it x} in insert mode}{:unma!\ {\it x}}
