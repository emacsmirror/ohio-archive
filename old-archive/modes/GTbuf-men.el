...
(defconst GTbuf-men-version (substring "$Revision: 1.45 $" 11 -2)
  "$Id: GTbuf-men.el,v 1.45 93/09/20 15:25:17 wurgler Exp $")

;; Copyright (C) 1991-1993 by Bill Benedetto and Tom Wurgler
...
;; LCD Archive Entry:
;; GTbuf-men|Tom Wurgler and Bill Benedetto|wurgler@gentire.com, benedett@gentire.com|
;; More dired-like buffer men (for v19)|
;; 20-Sep-1993|1.45|~/modes/GTbuf-men.el.Z|
;; Archived in: /pub/gnu/emacs/elisp-archive/modes/GTbuf-men.el.Z
...
;          %d - mark buffers for deletion containing regexp (on buffers)
;          %f - mark buffers for deletion containing regexp (on filenames)
...
;     C-n, n - go to the next buffer line and position on the name
;     C-p, p - go to the previous buffer line and position on the name
...
;
; This package is based on and requires Sebastian Kremer's dired code, 
; now included with emacs 19+.
;
...
(defvar buffer-menu-same-window nil
  "*If non-nil, keep the buffer-menu in the same window.")
...
Type \\[Buffer-menu-quit] immediately to make the buffer menu go away."
  ...
  (if (or current-prefix-arg (and buffer-menu-restricted fromp))
	...
	(if (not (string-equal buffer-menu-opts ""))
	    (progn
		  ...
		  (if (not buff-opt)
		      (setq buffer-menu-regexp (read-from-minibuffer
						"Regexp: "
						nil nil nil
 ...
 -- ------         ----  ----           ---------\n")
			...
			;; Lucid and Epoch 3.2 append the filename
			;; for dired buffers
			;; so does GNU Emacs 19
		  ...
		  (let ((buffer-read-only nil))
		    (Buffer-menu-goto-first-buffer)
		    (buffer-mark-lines-regexp buffer-menu-regexp "_")
		    (keep-lines "^_.. ")
		    (buffer-unflag-all-lines "_")
		    (message "")
		    (Buffer-menu-goto-first-buffer))))
	  (buffer-menu nil)
	  (goto-line 3)
  ...
  (make-variable-buffer-local 'buffer-menu-mode-options)
  (setq buffer-menu-restricted nil)
...
(defun Buffer-menu-buffer (error-if-non-existent-p)
  ...
  (save-excursion
    ...
    (let ((start (point))
	  string)
      (re-search-forward "\\s-+[0-9]+\\s-+[a-zA-Z]")
      (setq string (buffer-substring start (match-beginning 0)))
...

(defun buffer-mark-lines-regexp (regexp &optional marker-char files-p)
  "Mark all buffers matching REGEXP for use in later commands.
A prefix argument means to unmark them instead.
Optional MARKER-CHAR is the marker to use.
Optional files-p says to apply the REGEXP to the filenames rather
than to the buffernames."
  (interactive
	...
	(dired-mark-if
	 (and (not (eolp))		; empty line
		  ...
		  ((fn
		    (if files-p
			(or
			 (buffer-file-name (Buffer-menu-buffer nil))
			 (save-excursion
			   (set-buffer (Buffer-menu-buffer nil))
...
(defun buffer-flag-regexp-buffers (regexp)
  "In buffer-menu, flag all buffers containing the specified REGEXP
\(on the buffername\) for deletion.
Use `^' and `$' to anchor matches."
  (interactive (list (dired-read-regexp
		      "Flag for deletion (regexp on buffername): ")))
  (buffer-mark-lines-regexp regexp buffer-delete-marker))

(defun buffer-flag-regexp-files (regexp)
  "In buffer-menu, flag all buffers containing the specified REGEXP
\(on the filename\) for deletion.
Use `^' and `$' to anchor matches."
  (interactive (list (dired-read-regexp
		      "Flag for deletion (regexp on filename): ")))
  (buffer-mark-lines-regexp regexp buffer-delete-marker t))

(defun buffer-unflag-all-lines (flag &optional arg)
...
(defun simultaneous-find-buffer (buffer-list)
...
the height of the current window and the variable `window-min-height'."
  (let ((size (/ (if buffer-menu-full-screen (1- (frame-height))
		   (window-height)) (length buffer-list))))
...
(defun Buffer-menu-mark (arg)
  ...
  (interactive "p")
  (let ((num))
    (if arg (setq num arg) (setq num 1))
    (if (> num 0) 
	(while (> num 0)
	  (beginning-of-line)
	  (if (looking-at " [-M]")
...
;; the delete-other-windows seems to be broken in v19.19, this is a 
;; patched version of Buffer-menu-1-window that gets around the problem
;; of cursor positioning when switching buffers etc.
...
(defun Buffer-menu-1-window ()
  ...
  (let ((beg (window-start)))
    ...
    (set-window-start (get-buffer-window (buffer-name)) beg)))

;; this one is here to allow for same window buffer-menu, modified 
;; from the original
...
(define-key Buffer-menu-mode-map "%d" 'buffer-flag-regexp-buffers)
(define-key Buffer-menu-mode-map "%f" 'buffer-flag-regexp-files)
(define-key Buffer-menu-mode-map "%m" 'buffer-mark-lines-regexp)
(define-key Buffer-menu-mode-map "\M-\C-?" 'buffer-unflag-all-lines)
...
(define-key Buffer-menu-mode-map [down] 'buffer-menu-next-line)
(define-key Buffer-menu-mode-map [up] 'buffer-menu-previous-line)
