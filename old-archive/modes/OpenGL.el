...
;;   Created: June 1994
;;   Last modified: 1995/01/27 10:28:02
;;   Version: 0.3
;;   Keywords: C, C++, modes, OpenGL
;;   Tested for:
;;     XEmacs (Lucid GNU Emacs) >= 19.10 
;;     Must work with FSF GNU Emacs > 19 ;-)
;;   Ftp access: 
;;    ftp.cenatls.cena.dgac.fr:pub/export/OpenGL.el.Z (France only)
;;    archive.cis.ohio-state.edu:pub/gnu/emacs/elisp-archive/modes/OpenGL.el.Z
;;
;; LCD Archive Entry:
;; mode-compile|Heddy Boubaker|boubaker@dgac.fr|
;; Minor mode commands for editing OpenGL C/C++ code.|
;; 1995/01/27 10:28:02|0.3|~/modes/OpenGL.el.Z|
;;
...
;;  ========
;;
;;   Provides some few commands for editing OpenGL programs in C language.
;;
;;; Installation:
;;  =============
;;
...
;;             '(lambda ()
;;                (cond ((string-match "/\\([Oo]pen\\)?[Gg][Ll]/" (buffer-file-name))
;;                  (require 'OpenGL)
;;                  (OpenGL-minor-mode 1)
...
;;                  (define-key OpenGL-minor-mode-map "\C-cl"  'OpenGL-new-list)
;;                  (define-key OpenGL-minor-mode-map "\C-co"  'OpenGL-new-object)))))
;;   And load OpenGL-minor-mode as you whish.
;;
;;; Bug Reports:
;;  ===========
;;
...
;;  ===========
;;
;;   This file provides `OpenGL-minor-mode'. When active the following
;;  commands are availaible:
;;  `OpenGL-complete-symbol': provides interactive completion of all OpenGL
;;   symbols (constants, typedefs and functions from files GL/gl.h, GL/glu.h,
;;   GL/glx.h) plus symbols from the public domain additionnal library GLUT
;;   (GL/glut.h).
;;  `OpenGL-describe-symbol-briefly': print in minibuffer a short description of
;;   any OpenGL symbol (as described above), for example: if the symbol is a
;;   function print it's return parameter type and it's parameters
;;   (ex: "OpenGL Function: void glBegin (GLenum mode);")
;;  `OpenGL-new-list': insert a frame for editing glNewList/glEndList.
;;  `OpenGL-new-object': insert a frame for editing glBegin/glEnd.
;;
;;; WhatsNew:
;;  ==========
;;
;;    Clearing/Cleaning documentation.
;;    Bugs correction in `OpenGL-describe-symbol-briefly':
;;   the regexp match incomplete symbols, and re-search-backward is
;;   now silencious on error.
;;    Keymap bug correction and removing `OpenGL-bind-my-keys' and
;;   automatic key binding for Lucid Emacs.
...
;;  =====
;;
;;   A lot of things but what? any ideas ?
;;
;;; Ob Ad:
;;  ======
;;
;;   Use my package mode-compile to compile your programs (availaible at the
;;   same ftp places --in ~/misc/mode-compile.el.Z at elisp archives-- this
;;   one could be found).
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

...
(defvar OpenGL-add-menu-before "Buffers"
  "*Menu name where to add OpenGL menu before.
For XEmacs only.
If nil OpenGL will not add it's menu to the menubar.")
...
(defconst OpenGL-version "0.3"
  "Current version of OpenGL minor mode.
Not the OpenGL version ...")


(defconst OpenGL-help-address "boubaker@dgac.fr"
...
(defun  OpenGL-new-list ()
  ...
  (let ((beg (point))
    ...
    (setq end (point))
    (goto-char (+ beg 21))
    (ogl--indent-region beg end)
    ))


;;;###autoload
(defun  OpenGL-new-object ()
  "*Insert a glBegin/glEnd frame to edit."
  (interactive)
  (ogl--insert-maybe-newline)
  (let ((beg (point))
        (end))
...
OpenGL minor-mode provides some few commands to edit OpenGL based
C/C++ programs:
