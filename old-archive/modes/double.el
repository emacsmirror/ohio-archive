; `double.el' allows you to remap the keyboard so pressing a key once
; will insert one (user defined) string and pressing the same key twice
; will replace it with another (user defined) string.  This also work
; with isearch.
; 
; You can use `double.el' to provide a convenient way to enter national
; languages on a US keyboard for languages that only adds a few extra
; letters to the Latin alphabet, like Scandinavian or German, by putting
; the national keys at the correct place while leaving the original US
; keys accessible by pressing the key twice.  This technique is
; particularly useful for Scandinavian languages where the extra letters
; in these languages are not considered accented versions of the
; originals, since this makes using iso-acc.el and similar packages
; awkward.
; 
; The default mapping gives you Danish and Norwegian letters on a US
; keyboard, but you can easily define a map for Swedish or German
; locally.
; 
; `double.el' requires FSF Emacs 19.20 or later.
; 
; 
...
;; 03-Feh-1994|5.2|~/modes/double.el.Z|
;;
