       ...
       (save-excursion
	      (let (chap last-chapter
		     ...
		     (insert "\\node{"
			    ...
			    "}")
	(nodify-insert-to-cc) (insert "\\c This\n\t{"
			    "Top"
			    "}")
	(nodify-insert-to-cc) (insert "\\c Next\n\t{"
			    (nodify-get-name-at-line (cadr all))
			    "}")
	(nodify-insert-to-cc) (insert "\\c Prev\n\t{"
			    (car up-all)
			    "}")
	(nodify-insert-to-cc) (insert "\\c Up\n")
...
(defun nodify-insert-to-cc ()
  (let ((cc (current-column)))
     (if (< cc comment-column)
	 (insert (make-string (- comment-column cc) 32)))))
