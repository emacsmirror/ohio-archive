...
(mapcar
 ...
 '((mail-extr-address-syntax-table
   ...
   (mail-extr-address-text-syntax-table
    (?\000 ?\177 ".")
    (?\200 ?\377 "w")	;assume high-bit is word constituent
    (?\040 " ")
    (?\t " ")
    (?\r " ")
    (?\n " ")
...
;;	    (setq upper-case-flag t)
	 ...
	 ;; #### Perhaps better behavior here would be to leave the whole
	 ;; rest of the line intact, assuming it's a sentence of some kind
	 ;; like "face iz white! eyes iz tired!".
	 (t
	  (setq name-done-flag t)))
	...
	(if word-found-flag
      ...
      ;; If the last thing in the name is 2 or more periods (an elipsis) then
      ;; keep them and the preceeding word.  This is for the benefit of whole
      ;; sentences in the name field: it's better behavior than dropping the
      ;; last word of the sentence...
      (if (and (not suffix-flag) (looking-at "\\.+\\'"))
