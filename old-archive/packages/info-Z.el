;From utkcs2!emory!samsung!zaphod.mps.ohio-state.edu!tut.cis.ohio-state.edu!FOOBAR.COLORADO.EDU!grunwald Fri Jul 20 08:23:11 EDT 1990
;Article 2219 of gnu.emacs.bug:
;Path: utkcs2!emory!samsung!zaphod.mps.ohio-state.edu!tut.cis.ohio-state.edu!FOOBAR.COLORADO.EDU!grunwald
;>From: grunwald@FOOBAR.COLORADO.EDU (Dirk Grunwald)
;Newsgroups: gnu.emacs.bug
;Subject: compressed info files
;Message-ID: <9007192025.AA03780@foobar.Colorado.EDU>
;Date: 19 Jul 90 20:25:16 GMT
...
;
;
;The following version of 'info.el' automatically uncompresses info
;files, allowing you to compress your entire info tree. Since I just
;added the elisp info files, I thought this was worth while. On fast
;machines (DECstation), it's barely noticable and saves 50% of the
;space.
;
;It also handles uncompressed info files, so you can mix & match.
;
...
(defun Info-find-node (filename nodename &optional no-going-back)
  ...
  (if filename
	...
	(if (file-exists-p temp)
	    (setq filename temp)
	  (let
	      ((compressed (concat temp ".Z")))
	    (if (file-exists-p compressed)
		(setq filename compressed)
  ...
  (unwind-protect
	...
	(or (null filename)
	    ...
	    (let ((buffer-read-only nil))
		  ...
		  (info-compress-buffer (current-buffer) t))
...
(defun Info-read-subfile (nodepos)
  ...
  (let (lastfilepos
	...
	(let ((buffer-read-only nil))
	  ...
	  (widen)
	  (erase-buffer)
	  (if (file-exists-p lastfilename)
	      (insert-file-contents lastfilename)
	    (let
		((compressed (concat lastfilename ".Z")))
	      (if (file-exists-p compressed)
		  (progn
		    (insert-file-contents compressed)
		    (info-compress-buffer (current-buffer) t)))))
	  (set-buffer-modified-p nil)
...
(defconst info-compress-magic-regexp "\037\235\220"
  "Regexp that matches the magic number at the beginning of files created
by the compress(1) command.")

(defun info-compress-region (start end &optional undo)
...
(defun info-compress-buffer (&optional buffer undo)
  "Compress BUFFER.
...
Prefix arg (or second arg non-nil from a program) UNDO means uncompress."
  ...
  (save-excursion
    (set-buffer buffer)
    (info-compress-region (point-min) (point-max) undo)))
