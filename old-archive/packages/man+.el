...
(defconst Manual-topic-buffer t "\
...
(defconst Manual-buffer-view-mode t "\
...
(defconst Manual-match-topic-exactly nil "\
...
(defconst Manual-query-multiple-pages t "\
...
(defconst Manual-directory-list nil "\
	...
	(let* ((name (car (or fmtlist manlist)))
	  ...
	  (with-output-to-temp-buffer bufname
	    ...
	    (save-excursion
	      ...
	      (setq hook temp-buffer-show-hook)
...
(defun Manual-select-directories (dirlist section)
  (mapcar '(lambda (fmtdir)
	     (if (or (not section)
		     (string-match (concat (substr section 0 1) "/$") fmtdir))
		 fmtdir))
	  dirlist))
...
(defun Manual-select-man-pages (dirlist topic section exact)
  (let ((manlist (apply 'append		; this removes the nulls
			(mapcar '(lambda (dir)
				   (mapcar '(lambda (file)
						  ...
						  (concat dir file)))
					   (file-name-all-completions (concat topic (if section (concat "." section))) dir)))
				dirlist))))
