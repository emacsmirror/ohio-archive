...
;;;; 
;;;; PURPOSE
;;;; 	Specialized extensions to dismal to support spa.
;;;; TABLE OF CONTENTS
;;;;
...
dis-paired-regexps defines what matches between the rows."
  (interactive "sSubject data column to align (a letter): 
sModel data column to align (a letter): 
nStarting row: 
nEnding row: ")
               ...
               (predvalue (if (aref predseqresult i)
                             ...
                             (set-buffer old-buffer)
                              (dismal-get-val (car (aref predseqresult i))
                               ...
                               "nil")))
         (insert (format "%2s: %10s %10s  %15s  %25s\n"
                          i (aref obsseqresult i) (aref predseqresult i)
 ...
 (if (not dismal-middle-col)
     (error
   "Can't tell where data stops/model begins: you must set dismal-middle-col"))
 ...
 (if (not (y-or-n-p (format "Do you want the %s matches out of %s aligned? "
   ...
   (if (not (y-or-n-p (format "Do you want blank lines deleted? ")))
       nil
     (dismal-delete-blank-rows start-row (+ end-row new-rows))))
...
(defun dis-move-references-forward ()
  ...
  (while (< i (1- (length predseqresult)))
    ...
    (while (and (not my-quit-flag)
      ...
      ;; (message "Doing i= %s, lowpost= %s, flag= %s  finali= %s" i low-post-number my-quit-flag (length predseqresult))
      ;; (beep t) (sit-for 2)
...
(defun dis-align-columns ()
 ;; Align the pred-row with the obs-row looking across dismal-middle-col
 ...
 (while (<= i length-of-result)
   ...
   (let* ((pred-cell (aref predseqresult i))
          ...
          (p-row (if pred-cell (+ p-offset (dis-address-row pred-cell))))
          (o-row (if obs-cell (+ o-offset (dis-address-row obs-cell))))  )
...
(defun dis-choose-to-do-edits ()  ;(dis-choose-to-do-edits)
 ...
 (while (and (< i length-of-result) (not just-do-rest))
  (let* ((pred-cell (aref predseqresult i))
         (obs-cell (aref obsseqresult i))
         (p-row (if pred-cell  (dis-address-row pred-cell)))
         (o-row (if obs-cell   (dis-address-row obs-cell)))  )
  (if (not (and pred-cell obs-cell))
      nil
    ;; I'm happy to make user type CR to be sure.
    (dismal-set-mark (dis-address-row obs-cell) (dis-address-col obs-cell))
    (dismal-jump-to-cell (dis-address-row pred-cell)
                         (dis-address-col pred-cell))
    (setq do-edit
          (dismal-read-minibuffer
             (format
    "Align match %s/%s, row %s:<%s> with row %s:<%s>? (y/n/a accept the rest)"
                     (1+ i) length-of-result
                     o-row (dismal-get-val o-row (dis-address-col obs-cell))
                     p-row (dismal-get-val p-row (dis-address-col pred-cell)))
             nil "y"))
...
;looking across dismal-middle-col.   Returns how many rows it added."
; (interactive)
;
; (setq i 0)
; (setq p-offset 0)
