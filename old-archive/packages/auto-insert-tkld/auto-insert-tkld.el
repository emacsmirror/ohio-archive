...
;; $Date: 1994/08/09 12:50:07 $|$Revision: 1.20 $|~/packages/auto-insert.tar.Z|
...
;;     set mail-self-address to an appropriate value if wrong.
...
(defconst auto-insert-version (substring "$Revision: 1.20 $" 11 -2)
  "$Id: auto-insert-tkld.el,v 1.20 1994/08/09 12:50:07 tkld Exp tkld $

Report bugs to: Kevin Davidson <tkld@cogsci.ed.ac.uk>")

(defvar mail-self-address
	  ...
	  (while (not (eobp))
		...
		(progn
		  ...
		  (cond
		   ...
		   ((looking-at "a")
		    (auto-insert-substitute mail-self-address))
		   ((looking-at "u")
...
leading directory part."
  (concat dir (if (string= "/" (substring dir -1 nil))
		  ""
		"/")
	  file))
