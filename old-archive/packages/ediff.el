...
(defvar ediff-version "1.65c" "The current version of Ediff.")
(defvar ediff-date "August 8, 1994" "Date of last update.")  

;; LCD Archive Entry:
;; ediff|Michael Kifer|kifer@cs.sunysb.edu|
;; Visual interface to diff and patch.|
;; 08-Aug-1994|1.65c|~/packages/ediff.el.Z|
...
When called interactively, displays the version."
  (interactive)
  (if (interactive-p)
      (message "Ediff: version %s of %s" (ediff-version) ediff-date)
    ediff-version))
...
;;;###autoload
(if (and window-system ediff-want-default-menus
	 (ediff-frame-has-menubar) (not (ediff-if-lucid)))
  (progn
...
(if (and window-system ediff-want-default-menus
	...
	'("File with its Version via RCS ..." . rcs-ediff))
      (define-key menu-bar-ediff-menu [vc-ediff]
	'("File with its Version via VC ..." . vc-ediff))
...
(if (and window-system ediff-want-default-menus (ediff-frame-has-menubar)
	...
	'(""
	  ...
	  ["File with its version via VC ..."  vc-ediff t]
	  ["File with its version via RCS ..."  rcs-ediff t]))
      (defvar epatch-menu
	'(""
