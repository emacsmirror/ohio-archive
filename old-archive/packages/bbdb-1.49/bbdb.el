...
;;;
;;;  -----------------------------------------------------------------------
;;; |  There is a mailing list for discussion of BBDB: info-bbdb@lucid.com. |
;;; |  To join, send mail to info-bbdb-request@lucid.com (don't forget the  |
...
;;; |  bbdb-announce-request@lucid.com.  This is a very low volume list,    |
...
(defconst bbdb-version "1.49;  1-dec-93.")
...
(defmacro bbdb-y-or-n-p (&rest args)
  (list 'prog1
	(cons 'y-or-n-p args)
	'(message " ")))
...
(defmacro bbdb-error-retry (form)
  (list 'let '(--bbdb-error-retry--)
    (list 'while
      (list 'not (list 'condition-case '--c--
		       (list 'progn
			     (list 'setq '--bbdb-error-retry-- form)
			     't)
		   '(error (ding)
...
(defmacro bbdb-with-db-buffer (&rest body)
  (cons 'bbdb-save-buffer-excursion
	(cons '(set-buffer (find-file-noselect bbdb-file 'nowarn))
...
(defmacro bbdb-hashtable ()
  '(bbdb-with-db-buffer (bbdb-records t) bbdb-hashtable))

(defun bbdb-changed-records ()
  (bbdb-with-db-buffer (bbdb-records t) bbdb-changed-records))
...
(defun bbdb-records (&optional already-in-db-buffer)
  "Return a list of all bbdb records; read in and parse the db if necessary.
This also notices if the disk file has changed out from under us."
  ...
  (let ((inside-bbdb-records t)
    ...
    (bbdb-save-buffer-excursion
      ...
      (let ((buf (if already-in-db-buffer
		     ...
		     (set-buffer (find-file-noselect bbdb-file 'nowarn)))))
	;; make sure the BBDB in memory is not out of synch with disk.
	(cond ((verify-visited-file-modtime buf) nil)
	      ((and bbdb-auto-revert-p
	       ...
	       (revert-buffer t t))
	      ;; hassle the user
	      ((yes-or-no-p (if (buffer-modified-p buf)
...
(defun bbdb-modified-p ()
  (setq bbdb-modified-p
	(buffer-modified-p (find-file-noselect bbdb-file 'nowarn))))
...
(cond ((and (string-match "Lucid" emacs-version)
	    (not (string-lessp emacs-version "19.3")))
