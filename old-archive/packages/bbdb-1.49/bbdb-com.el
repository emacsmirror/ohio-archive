...
(defun bbdb-delete-current-record (r &optional noprompt)
  ...
  (if (or noprompt
	  (prog1 (y-or-n-p (format "delete the entire db entry of %s? "
  ...
  (let*(extra-name
	(name
	 (cond ((and (/= 0 (length (bbdb-record-name old-record)))
		...
		(prog1
		    (if (y-or-n-p (format "Use name \"%s\" instead of \"%s\"? "
	...
	(comp
	 (cond ((and (/= 0 (length (bbdb-record-company old-record)))
		...
		(if (y-or-n-p (format "Use company \"%s\" instead of \"%s\"? "
...
address is used as-is."
  ...
  (let* ((override (bbdb-record-getprop record 'mail-name))
	 ...
	 fn ln both (i 0))
    (if override
	(setq both (bbdb-divide-name override)
	      fn (car both)
	      ln (car (cdr both)))
    ...
    (cond ((or (null name)
	       (cond ((and fn ln)
		     ...
		     ((or fn ln)
		      (string-match
		       (concat "\\`[^!@%]*\\b" (regexp-quote (or fn ln))
			       "\\b[^!%@]")
		       (or fn ln))))
...
Otherwise, a valid response is forced."
  (let* ((ht (bbdb-hashtable))
	 (string (completing-read prompt ht 'bbdb-completion-predicate))
