From mailrus!ukma!uflorida!novavax!weiner@bbn.com Fri May 19 14:36:46 1989
To: unix-emacs@bbn.com
Date: 28 Apr 89 06:19:13 GMT
From: Bob Weiner <mailrus!ukma!uflorida!novavax!weiner@bbn.com>
Sender: arpa-unix-emacs-request@bbn.com
Subject: rmail.el, improved, see header for changes, get rmailsum.el following
...
;; Modified by Bob Weiner, Motorola Inc., 2/23/89
;;  Changed 'e' key binding from 'rmail-expunge' (which 'x' performs in
;;  this and every mode) to 'rmail-edit-current-message' since an edit
;;  function is a more common binding for 'e' in read only modes, e.g. Info.
;;
;; Bob Weiner, Motorola, Inc., 4/7/89
;;  Modified 'rmail-next-undeleted-message' so that when the first or last
;;  message is deleted the next or previous message, respectively is displayed.
;;  This is more in keeping with the way the delete message functions work when
;;  not at the beginning or end of the message list.
;;
;;  Modified 'rmail-delete-forward' so that it puts a 'D' tag on the proper
;;  line in summary buffer if this buffer exists.  Modified
;;  'rmail-undelete-previous-message' so that it removes the 'D' from the
;;  summary buffer.
;;
;; Bob Weiner, Motorola, Inc., 4/11/89
;;  Modified 'rmail-expunge' and 'rmail-show-message' to update summary buffer
;;  also if it is currently displayed.
;;
;; Bob Weiner, Motorola, Inc., 4/25/89
;;  Modified 'rmail-get-new-mail' so it updates summary buffer, if one exists.
...
e	Edit the current message.  C-c C-c to return to Rmail.
  ...
  (if (zerop rmail-total-messages)
	...
	(if rmail-summary-buffer
	    (let ((curr-msg rmail-current-message))
	      (funcall
		(if (get-buffer-window rmail-summary-buffer)
		    'pop-to-buffer
		  'set-buffer)
...
(defun rmail-next-undeleted-message (n &optional no-reverse)
"Display next non-deleted message.
With prefix argument N, moves forward this number of non-deleted messages or
backward if n is negative.  If no non-deleted message is found and the current
message is deleted, the next non-deleted message in the opposite direction is
found regardless of the value of n, unless optional second argument
NO-REVERSE is non-nil."
  (interactive "p")
  (rmail-maybe-set-message-counters)
  (let ((count (if (< n 0) (- n) n))
	(msg-to-goto rmail-current-message)
	(current (+ rmail-current-message (if (> n 0) 1 -1)))
	(curr-incr (if (< n 0) '1- '1+))
	(non-del-msg-found nil))
    (while (and (> count 0) (> current 0) (<= current rmail-total-messages))
      (if (rmail-message-deleted-p current)
	  nil
	(setq non-del-msg-found t)
	(setq msg-to-goto current)
	(setq count (1- count)))
      (setq current (funcall curr-incr current)))
    (if (or no-reverse non-del-msg-found
	    (not (rmail-message-deleted-p msg-to-goto)))
	(if (= msg-to-goto rmail-current-message)
	    (message "No nondeleted message available")
	  (rmail-show-message msg-to-goto))
      (rmail-next-undeleted-message (if (> n 0) -1 1) t))))
...
(defun rmail-expunge ()
  "Actually erase all deleted messages in the file and summary buffer if displayed."
  (interactive)
  (if (and rmail-summary-buffer
	   (get-buffer-window rmail-summary-buffer))
      (progn (pop-to-buffer rmail-summary-buffer)
