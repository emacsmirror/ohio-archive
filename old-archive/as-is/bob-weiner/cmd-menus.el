From apple!bloom-beacon!tut.cis.ohio-state.edu!ukma!uflorida!novavax!weiner@bbn.com Fri May 19 14:35:51 1989
To: unix-emacs@bbn.com
Date: 28 Apr 89 06:14:13 GMT
From: Bob Weiner <apple!bloom-beacon!tut.cis.ohio-state.edu!ukma!uflorida!novavax!weiner@bbn.com>
Sender: arpa-unix-emacs-request@bbn.com
Subject: Command reference (menus) extensions to GNU Emacs
...
;; Modified on 18-Oct-88 by Leonard N. Zubkoff:
;;  Added code for info node creation.

;; Bob Weiner, Motorola, Inc., 3/28/89
;;  Added constant definitions for 'Info-exec-only-node-ref-char' and for
;;  'Info-exec-no-node-ref-char'.
;;  The first defines a character prepended to a command menu entry that
;;  indicates that the code associated with the entry's node should be executed
;;  but the node should not be visited.
;;  The second defines a character prepended to a command menu entry
;;  or cross reference that indicates that a the command given by the
;;  variable 'Info-cmd-menu-expression' should be executed with the
;;  entry name as an argument, but no node should be visited when such
;;  an entry is selected.
;;
;;  Modified 'Info-goto-node' so that node names whose menu entries or
;;  cross-reference entries begin with 'Info-exec-only-node-ref-char' are
;;  executed only, they are not visited.  This allows Info to support command
;;  menus well.
;;
;;  Also added exec-only-node flag to 'Info-find-node' and added
;;  'Info-get-node-expression' function to support this functionality.
;;
;; Bob Weiner, Motorola, Inc., 3/29/89
;;  Added 'Info-cmd-menu-expression' variable definition.  It's value is set by
;;  Info command menus whose entries that begin with
;;  'Info-exec-no-node-ref-char' execute this code.  No nodes exist or are
;;  visited for such entries.  Modified 'Info-goto-node' to handle this type of
;;  entry. 
;;
;;  Added 'info-mode-hook' variable which specifies expression to run at the
;;  end of the 'Info-mode' function.
...
(defconst Info-exec-only-node-ref-char "!"
  "Single character string indicating an execute-only Info node reference.")

(defconst Info-exec-no-node-ref-char "@"
  "Single character string indicating an execute-only Info command menu entry
or cross-reference.  There is no node associated with such an entry.  The node
in which the reference is made should set 'Info-cmd-menus-expression' to a
function that takes one argument, the reference name.")

(defvar Info-cmd-menu-expression nil
  "Expression to evaluate when an Info menu entry without a node is selected.
Such entry names begin with the character given by 'Info-exec-no-node-ref-char'.
The nodename within the entry, minus this initial character, is sent as the
sole argument to this expression.")
...
;; it says do not attempt further (recursive) error recovery.  EXEC-ONLY-NODE is
;; non-nil if code within node should be executed but node should not be
;; visited.
(defun Info-find-node (filename nodename &optional no-going-back exec-only-node)
  ...
  (let ((active-expression nil))
	...
	(progn
	  ...
	  (if (equal nodename "*")
		...
		(error "No such node: %s" nodename)))
	    (if exec-only-node
		(setq active-expression (Info-get-node-expression))
	      (Info-select-node))))
	  ...
	  (let ((hist (car Info-history)))
	    ...
	    (if exec-only-node
		(if Info-enable-active-nodes
		    (eval active-expression)
		  (error "Info expression evaluation disabled, set 'Info-enable-active-nodes' non-nil to enable.")))))))
  (if exec-only-node
      nil
    (goto-char (point-min))))
...
;; Return the Elisp code within the info node that point is in;
;; don't select node.
(defun Info-get-node-expression ()
  (save-excursion
...
'@NODENAME' means execute function given by 'Info-cmd-menu-expression', do not visit
node.  '!NODENAME' means evaluate expression from node but do not visit it." 
  (interactive "sGoto node: ")
  (let ((exec-only-menu-entry (string= (substring nodename 0 1)
				       Info-exec-no-node-ref-char))
	(exec-only-node (string= (substring nodename 0 1)
				 Info-exec-only-node-ref-char)))
    (cond (exec-only-menu-entry
	    (if Info-enable-active-nodes
		(funcall Info-cmd-menu-expression (substring nodename 1))
	      (error "Info expression evaluation disabled, set 'Info-enable-active-nodes' non-nil to enable.")))
	  (exec-only-node
	    (setq nodename (substring nodename 1))))
    (if (not exec-only-menu-entry)
	(let (filename)
...
(if Info-mode-map
  ...
  (define-key Info-mode-map "i" 'Info-create-node)
...
;;; Create a new node in an info file

(defun Info-create-node (node &optional disjoint)
  "Create a new node in an info tree.
   NODE is the name of the new node to create.  Prefix arg (second arg if
   noninteractive) means don't link the Prev, Next and Up pointers.
   Otherwise, the new node is linked into the info structure in a
   reasonable manner."
  (interactive "sCreate node: ")
  (let* ((curr (if (or disjoint current-prefix-arg) nil Info-current-node))
	 (up (Info-get-pointer curr "up"))
	 (next (Info-get-pointer curr "next"))
	 (prev (Info-get-pointer next "prev")))
    (Info-create-empty-node node (Info-get-pointer Info-current-node "file"))
    (Info-set-pointer node "Prev" curr)
    (Info-set-pointer node "Up" up)
    (Info-set-pointer node "Next" next)
    (Info-set-pointer curr "Next" node)
    ;; Only link in PREV(NEXT) if it pointed to the current node
    (if (equal prev curr) (Info-set-pointer next "Prev" node))
    (Info-goto-node node)
    (goto-char (point-max))))

(defun Info-create-empty-node (node &optional file)
  "Create an uninitialized node following the current node."
  (goto-char (point-max))
  (setq buffer-read-only nil)
  (insert "\n\^_\n")
  (if file (insert "File: " file ",  "))
  (insert "Node: " node "\n\n")
  (setq buffer-read-only t)
  (Info-select-node))

(defun Info-get-pointer (node-name pointer-name)
  "Get the value of pointer-name in the node node-name.  Returns NIL
   if pointer not found or if either of node-name or pointer-name is
   NIL."
  (if (and node-name pointer-name)
      (save-excursion
	(Info-goto-node node-name)
	(goto-char (point-min))
	(forward-line 1)
	(if (re-search-backward (concat pointer-name ":") nil t)
	    (progn
	      (goto-char (match-end 0))
	      (Info-following-node-name))
	  nil))))

(defun Info-delete-pointer (node-name pointer-name)
  "Delete the named pointer in the named node.  This is a no-op if
   the node-name or pointer-name are nil, or if the named pointer
   doesn't exist in that node."
  (if (and node-name pointer-name)
      (save-excursion
	(Info-goto-node node-name)
	(goto-char (point-min))
	(forward-line 1)
	(if (re-search-backward (concat pointer-name ":") nil t)
	    ;; Found the pointer - delete the pointer name and value
	    (let ((start (progn
			   ;; This leaves a leading comma if we try do
			   ;; delete the first pointer field in header,
			   ;; but that won't usually happen.
...
(defun Info-set-pointer (node-name pointer-name pointer-value)
  "Set a pointer in the header of the named node.  If pointer-value
   is NIL, the named node is deleted.  If either node-name or
   pointer-name are nil, then this function is a no-op.  If the
   pointer already exists, its value is changed.  If the pointer
   doesn't already exist, it is created at the end of the pointer list."
  (if (and node-name pointer-name)
      (if (null pointer-value)
	  (Info-delete-pointer node-name pointer-name)
	(save-excursion
	  (Info-goto-node node-name)
	  (setq buffer-read-only nil)
	  (goto-char (point-min))
	  (forward-line 1)
	  (if (re-search-backward (concat pointer-name ":") nil t)
	      ;; found the named pointer
	      (let ((start (match-end 0))
		    (end (progn
			   (skip-chars-forward " \t")
			   (skip-chars-forward "^,\t\n")
			   (point))))
		;; out with the old value
		(delete-region start end)
		;; in with the new
	    ...
	    (progn
	      (end-of-line)
	      (backward-char 1)
	      (insert ",  " pointer-name ": " pointer-value)))
	  (setq buffer-read-only t)))))
