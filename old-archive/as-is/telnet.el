...
Subject: better password handling in telnet.el
Date: Tue, 21 Feb 89 18:44:33 -0500
From: Stephen Gildea <gildea@bbn.com>

Here is a version of telnet.el that handles the initial password
inputting better than the version in 18.52 does.  Some systems send
the initial banner message in lots of little packets, so this version
looks longer for the password prompt.  There are new variables to make
this timing customizable.  Also, the password match is more specific
so it doesn't get confused by "incorrect password" messages.

This version also includes Hal Peterson's telnet-last-input fixes.
...
(defvar telnet-count 0
  "Number of output strings read from the telnet process whil looking
...
(defvar telnet-maximum-count 4
  "Maximum value telnet-count can have.  After this many passes, we
stop looking for initial setup data.  Should be set to the number of
terminal writes telnet will make rejecting one login and prompting
for the again for a username and password.")
