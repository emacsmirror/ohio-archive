...
;Date: 25 Jan 89 13:51:16 GMT
;From: Richard Hess <cimshop!rhess@uunet.uu.net>
;Subject: [Re:] reading netnews in GNU... [ an enhanced version of rnews ]
;
;
;  The following is an enhanced version of rnews which supports a summary mode
;mechanism for browsing large news groups.  The "s" key  is used in news-mode to
;generate the summary buffer and in summary-mode to select an article.  Look at
;news-mode and summary-mode for more details of the enhancements...
;
;-------------------------------------------------------------------------------
...
;
; [modified]:  rhess  12-27-88  [ news summary support... ]
; -----------------------------------------------------------------------------
;
...
s       view summary info on <new> messages in current newsgroup... [rhess]
S       create summary info on new messages in current newsgroup... [rhess]
T       view summary info on all messages in current newsgroup... [rhess]
...
(defun news-get-new-news ()
  ...
  (interactive)
  (news-kill-summary-buffer)  ; [rhess]
...
(defun news-force-update ()
  ...
  (interactive)
  (news-force-update-internal)
  (message "Updated to %d" news-current-message-number))

(defun news-force-update-internal ()
  "[ Internal ]:  core functionality of news-force-update... [ rhess ]"
...
; -----------------------------------------------------------------------------
;  original:  {...}!uunet!cimshop!rhess
; -----------------------------------------------------------------------------
;  based on [ news-summary ] by Dave Platt, {...}!uunet!coherent!dplatt
; -----------------------------------------------------------------------------

(defvar news-sbuf-name "*news* summary"
  "Name of buffer into which summary information is generated... [rhess]")

(defun news-kill-summary-buffer ()
  "Remove any window(s) displaying the summary buffer, and kill it..."
  (let ((sbuf (get-buffer news-sbuf-name)))
       (if (bufferp sbuf)
         (progn
          (delete-windows-on sbuf)
          (kill-buffer sbuf)
	  (setq news-sbuf-name "*news* summary")
	  ))))
...
(defun news-create-sbuf ()
  "Creates the raw buffer for current news group summary info... [ rhess ]"
  (setq news-sbuf-name (format " *%s" news-current-news-group))
  (generate-new-buffer news-sbuf-name))

(defun news-summary-core (init-article)
  "[ CORE ]:  creates summary of news articles... [ rhess ]"
  (let* ((sbuf (or (get-buffer news-sbuf-name)
                   (news-create-sbuf)))
	 (rbuf (current-buffer))
...
; -----------------------------------------------------------------------------
;  based on [ View ]:  Principal author K. Shane Hartman
; -----------------------------------------------------------------------------
...
(defun summary-mode-command-loop ()
  (let ((old-local-map (current-local-map))
    ...
    (unwind-protect  (progn  (use-local-map summary-mode-map)
...
s		select rnews article on current line... [ rhess ]
n		next line.
p		previous line.
.		set the mark.
...
;  (interactive)
  ...
  (unwind-protect
	...
	(if summary-current-article
	    (progn (goto-char 1)
		   (if (not (eobp))
		       (search-forward (int-to-string summary-current-article))
		     )
		   (beginning-of-line)))
	...
	(progn
	  ...
	  (news-select-message summary-selected-article)
	  (news-force-update-internal)
	  ))
    ))
...
(defun Summary-select-article ()
  "Select current article at point... [ rhess ]"
  (interactive)
